...
class axi_master_monitor extends uvm_monitor;
    ...
    function axi_transaction create_write_transaction();
        ...
        tr = axi_transaction::type_id::create("ext_write_tr", this);
    ...
    function axi_transaction create_read_transaction();
        ...
        tr = axi_transaction::type_id::create("ext_read_tr", this);
        ...
        forever begin
            ...
            if (vif.mon_cb.wvalid && vif.mon_cb.wready) begin
                if (write_addr_q.size() == 0) begin
                    `uvm_error("AXI_MON", "External master monitor observed W beat with no queued AW")
                end else begin
                    tr = write_addr_q[0];
                    ...
                    if (beat_idx >= beats) begin
                        `uvm_error("AXI_MON", $sformatf("External master monitor observed extra W beat id=%0d addr=0x%08h",
                                   tr.id, tr.addr))
                    end else begin
                        ...
                        if (vif.mon_cb.wlast !== ((beat_idx + 1) == beats)) begin
                            `uvm_error("AXI_MON", $sformatf("External master monitor WLAST mismatch id=%0d addr=0x%08h beat=%0d beats=%0d wlast=%0b",
            ...
            if (vif.mon_cb.bvalid && vif.mon_cb.bready) begin
                ...
                if (write_done_by_id[id].size() == 0) begin
                    `uvm_error("AXI_MON", $sformatf("External master monitor observed B response with no completed write id=%0d resp=%0b",
                               id, vif.mon_cb.bresp))
                end else begin
        ...
        forever begin
            ...
            if (vif.mon_cb.rvalid && vif.mon_cb.rready) begin
                ...
                if (read_by_id[id].size() == 0) begin
                    `uvm_error("AXI_MON", $sformatf("External master monitor observed R beat with no queued AR id=%0d resp=%0b",
                               id, vif.mon_cb.rresp))
                end else begin
                    tr = read_by_id[id][0];
                    ...
                    if (beat_idx >= beats) begin
                        `uvm_error("AXI_MON", $sformatf("External master monitor observed extra R beat id=%0d addr=0x%08h",
                                   tr.id, tr.addr))
                    end else begin
                        ...
                        if (vif.mon_cb.rlast !== ((beat_idx + 1) == beats)) begin
                            `uvm_error("AXI_MON", $sformatf("External master monitor RLAST mismatch id=%0d addr=0x%08h beat=%0d beats=%0d rlast=%0b",
