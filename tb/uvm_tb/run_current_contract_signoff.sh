...
if [ "$p3a_dir_tot" -ne 3 ] || [ "$p3a_dir_pass" -ne 3 ] || [ "$p3a_dir_fail" -ne 0 ]; then
    fail_signoff "REGRESSION_CARDINALITY" "Phase 3A directed gate cardinality mismatch: total=$p3a_dir_tot (expected 3), passed=$p3a_dir_pass (expected 3), failed=$p3a_dir_fail (expected 0)"
fi
if [ "$p3a_cov_tot" -ne 3 ] || [ "$p3a_cov_pass" -ne 3 ] || [ "$p3a_cov_fail" -ne 0 ]; then
    fail_signoff "REGRESSION_CARDINALITY" "Phase 3A coverage gate cardinality mismatch: total=$p3a_cov_tot (expected 3), passed=$p3a_cov_pass (expected 3), failed=$p3a_cov_fail (expected 0)"
fi
