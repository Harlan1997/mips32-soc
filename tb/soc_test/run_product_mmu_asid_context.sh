...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/soc_test/product_mmu_asid_context"}
mkdir -p "${RUN_DIR}"
...
make -C "${ROOT_DIR}/tb/soc_test/fw" \
    FW_NAME=mmu_asid_context OUT_DIR="${FW_DIR}" FW_BASE=firmware all
...
vcs -full64 -sverilog -timescale=1ns/1ps \
    ...
    "${ROOT_DIR}"/tb/unit/bootrom/tb_product_mmu_asid_context.sv -l compile.log

./simv -no_save "${TRACE_ARG[@]}" +FW_HEX="${FW_DIR}/firmware.hex" +BOOT_ROM_HEX="${FW_DIR}/firmware.hex" -l sim.log
grep -q "REGRESSION_TEST_SUCCESS product_mmu_asid_context" sim.log
