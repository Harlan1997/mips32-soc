...
timeout "${QEMU_TIMEOUT:-10}" "${QEMU_BIN}" -M "mips32-soc-ref,qspi-image=${FW_DIR}/firmware.elf" -m 64K -kernel "${FW_DIR}/firmware.elf" -nographic -monitor none >"${RUN_DIR}/qemu_stdout.log" 2>"${RUN_DIR}/qemu_stderr.log"
