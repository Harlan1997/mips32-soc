...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/soc_test/product_mmu_machine_check"}
mkdir -p "${RUN_DIR}"
...
make -C "${ROOT_DIR}/tb/soc_test/fw" \
    FW_NAME=mmu_machine_check OUT_DIR="${FW_DIR}" FW_BASE=firmware all
...
vcs -full64 -sverilog -timescale=1ns/1ps \
    ...
    +define+SOC_MICRO_TLB_ENABLE=1 \
    +incdir+"${ROOT_DIR}/rtl/include" +incdir+"${ROOT_DIR}/rtl/cpu" \
    +incdir+"${ROOT_DIR}/rtl/axi" +incdir+"${ROOT_DIR}/rtl/perips" \
    "${ROOT_DIR}"/rtl/cpu/*.v "${ROOT_DIR}"/rtl/axi/*.v \
    "${ROOT_DIR}"/rtl/perips/*.v "${ROOT_DIR}"/rtl/cache/*.v \
    ...
    "${ROOT_DIR}"/tb/unit/bootrom/tb_product_mmu_machine_check.sv -l compile.log

./simv -no_save +FW_HEX="${FW_DIR}/firmware.hex" \
    +BOOT_ROM_HEX="${FW_DIR}/firmware.hex" -l sim.log
grep -q "REGRESSION_TEST_SUCCESS product_mmu_machine_check" sim.log
