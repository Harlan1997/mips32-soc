        ...
        function void write(mem_transaction tr);
            ...
            if (tr.mem_write) begin
                case (tr.mem_op)
                    3'b000: begin // SB
                        ...
                        case (align)
                            2'b00: exp_dmem_we = 4'b0001;
                            2'b01: exp_dmem_we = 4'b0010;
                            2'b10: exp_dmem_we = 4'b0100;
                            2'b11: exp_dmem_we = 4'b1000;
                        endcase
                    end
                    3'b010: begin // SH
                        exp_dmem_wdata = {2{tr.mem_val_rt[15:0]}};
                        case (align[1])
                            1'b0: exp_dmem_we = 4'b0011;
                            1'b1: exp_dmem_we = 4'b1100;
                        endcase
                    end
                    3'b100: begin // SW
            ...
            if (tr.mem_read) begin
                case (tr.mem_op)
                    ...
                    3'b100: exp_mem_rdata_ext = tr.dmem_rdata;
                endcase
            end
            ...
            if (tr.dmem_addr !== exp_dmem_addr || tr.dmem_en !== exp_dmem_en || tr.dmem_wdata !== exp_dmem_wdata ||
                tr.dmem_we !== exp_dmem_we || tr.mem_rdata_ext !== exp_mem_rdata_ext || 
                tr.adel_exception !== exp_adel || tr.ades_exception !== exp_ades) begin
                `uvm_error("MEM_MISMATCH", $sformatf("Mismatch: \nInputs: addr=%h op=%b r=%b w=%b \nExpected: en=%b we=%b wdata=%h rdata_ext=%h adel=%b ades=%b \nActual: en=%b we=%b wdata=%h rdata_ext=%h adel=%b ades=%b", 
                    tr.mem_ex_out, tr.mem_op, tr.mem_read, tr.mem_write, 
                    exp_dmem_en, exp_dmem_we, exp_dmem_wdata, exp_mem_rdata_ext, exp_adel, exp_ades,
                    tr.dmem_en, tr.dmem_we, tr.dmem_wdata, tr.mem_rdata_ext, tr.adel_exception, tr.ades_exception))
                error_count++;
            end else begin
                match_count++;
            end
