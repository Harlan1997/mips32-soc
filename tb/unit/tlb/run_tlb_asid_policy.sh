...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/unit/tlb_asid_policy"}
mkdir -p "${RUN_DIR}"
cd "${RUN_DIR}"
...
if grep -q "REGRESSION_TEST_SUCCESS tlb_asid_policy" sim.log; then
    echo "tlb_asid_policy: PASS"
else
    echo "tlb_asid_policy: FAIL"
