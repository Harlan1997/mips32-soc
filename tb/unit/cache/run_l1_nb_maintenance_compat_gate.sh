...
# The opt-in adapter routes the supported D-cache invalidate, tag and
# writeback operations to L1, while retaining the legacy dcache for
# unsupported operations. Make that split executable alongside the CPU-side
# contracts.
...
- `Index_Invalidate_D`, `Hit_Invalidate_D`, tag operations and supported
  writeback operations are accepted by the opt-in L1 only after outstanding
  line traffic and responses drain, then complete with a one-cycle L1
...
This closes the supported D-cache maintenance subset in the opt-in L1
adapter. It does not claim unsupported CACHE encodings, concurrent
