...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/unit_tb/product_fetch_pc_alignment"}
...
vcs -full64 -sverilog -timescale=1ns/1ps \
    ...
    "${SCRIPT_DIR}"/tb_fetch_pc_alignment.sv -l compile.log

./simv -no_save \
    +FW_HEX="${ROOT_DIR}/tb/uvm_tb/data/flash_xip_image.hex" \
    +BOOT_ROM_HEX="${SCRIPT_DIR}/data/fetch_pc_alignment.hex" -l sim.log
grep -q "REGRESSION_TEST_SUCCESS fetch_pc_alignment" sim.log
