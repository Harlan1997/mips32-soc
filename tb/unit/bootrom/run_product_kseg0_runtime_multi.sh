...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/unit_tb/product_kseg0_runtime_multi"}
...
make -C "${ROOT_DIR}/tb/soc_test/fw/tests/boot_manifest_handoff" \
    OUT_DIR="${FW_DIR}" FW_BASE=firmware multi-segment
...
./simv -no_save \
    ...
    +SPI_FLASH_HEX="${FW_DIR}/flash_image_multi.hex" \
    +EXPECT_KSEG0_RUNTIME_MULTI \
    -l sim.log

grep -q "REGRESSION_TEST_SUCCESS product_manifest_handoff_valid" sim.log

./simv -no_save \
    +BOOT_ROM_HEX="${FW_DIR}/boot_rom.hex" \
    +SPI_FLASH_HEX="${FW_DIR}/flash_image_multi_wx.hex" \
    +EXPECT_KSEG0_RUNTIME_MULTI \
    +EXPECT_BOOT_FAILURE +EXPECT_RUNTIME_FAILURE +EXPECT_FAILURE_NAME=multi_wx \
    -l sim_multi_wx.log
grep -q "REGRESSION_TEST_SUCCESS product_manifest_handoff_multi_wx" sim_multi_wx.log
