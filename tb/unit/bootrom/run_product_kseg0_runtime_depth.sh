...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/unit_tb/product_kseg0_runtime_depth"}
...
make -C "${ROOT_DIR}/tb/soc_test/fw/tests/boot_manifest_handoff" \
    OUT_DIR="${FW_DIR}" FW_BASE=firmware all depth
...
./simv -no_save \
    ...
    +SPI_FLASH_HEX="${FW_DIR}/flash_image_depth.hex" \
    +EXPECT_KSEG0_RUNTIME +EXPECT_KSEG0_DEPTH \
    -l sim_depth.log
grep -q "REGRESSION_TEST_SUCCESS product_manifest_handoff_valid" sim_depth.log
