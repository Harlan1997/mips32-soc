...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/unit_tb/product_mmu_ebase_modified"}
...
make -C "${ROOT_DIR}/tb/soc_test/fw" \
    FW_NAME=mmu_ebase_modified OUT_DIR="${FW_DIR}" FW_BASE=firmware all
...
vcs -full64 -sverilog -timescale=1ns/1ps \
    ...
    "${SCRIPT_DIR}"/tb_product_mmu_ebase_modified.sv -l compile.log

./simv -no_save \
    +FW_HEX="${FW_DIR}/firmware.hex" \
    +BOOT_ROM_HEX="${FW_DIR}/firmware.hex" -l sim.log
grep -q "REGRESSION_TEST_SUCCESS product_mmu_ebase_modified" sim.log
