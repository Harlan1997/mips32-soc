// Integrated CPU/I-cache execution gate.
// The program jumps through six distinct tags mapping to one I-cache set.
// The fifth/sixth line must evict a prior line, causing a later refill.
`timescale 1ns/1ps

module tb_mips_core_icache_exec;
    reg clk = 1'b0;
    reg rst_n = 1'b0;
    always #5 clk = ~clk;
    ...
    function [31:0] program_word(input [31:0] addr);
        begin
            ...
            case (addr & 32'hffff_ffe0)
                32'h0000_0000: if (addr[4:2] == 3'd0) program_word = 32'h0800_0200;
                32'h0000_0800: if (addr[4:2] == 3'd0) program_word = 32'h0800_0400;
                32'h0000_1000: if (addr[4:2] == 3'd0) program_word = 32'h0800_0600;
                32'h0000_1800: if (addr[4:2] == 3'd0) program_word = 32'h0800_0800;
                32'h0000_2000: if (addr[4:2] == 3'd0) program_word = 32'h0800_0a00;
                32'h0000_2800: if (addr[4:2] == 3'd0) program_word = 32'h0800_0000;
        ...
        if (!rst_n) begin
            ...
            if (inst_arvalid && inst_arready) begin
                case (inst_araddr)
                    ...
                    32'h0000_1000: begin if (seen_mask[2]) repeat_seen <= 1'b1; else seen_mask[2] <= 1'b1; end
                    32'h0000_1800: begin if (seen_mask[3]) repeat_seen <= 1'b1; else seen_mask[3] <= 1'b1; end
                    32'h0000_2000: begin if (seen_mask[4]) repeat_seen <= 1'b1; else seen_mask[4] <= 1'b1; end
                    32'h0000_2800: begin if (seen_mask[5]) repeat_seen <= 1'b1; else seen_mask[5] <= 1'b1; end
                    default: ;
                endcase
            end
