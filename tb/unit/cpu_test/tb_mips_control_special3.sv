...
module tb_mips_control_special3;
    ...
    initial begin
        ...
        // Standard MIPS32 R2 RDHWR uses rs=0 and places the hardware
        // register selector in rd (the old rs=3 fixture was non-standard).
        expect_valid(special3(5'd0, 5'd2, 5'd2, 5'd0, 6'b111011));
        ...
        expect_bshfl_valid(5'b00110, 5'b11100); // WSBW
        for (integer subop = 0; subop < 32; subop = subop + 1) begin
            if (subop != 0 && subop != 2 && subop != 6 && subop != 8 && subop != 9 &&
