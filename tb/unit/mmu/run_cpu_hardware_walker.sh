...
RUN_DIR=${RUN_DIR:-"${ROOT_DIR}/build/unit_tb/cpu_hardware_walker"}; mkdir -p "${RUN_DIR}"; cd "${RUN_DIR}"
source /etc/profile.d/modules.sh; module load vcs
VCS_DEFINES=${VCS_DEFINES:-}
vcs -full64 -sverilog -timescale=1ns/1ps ${VCS_DEFINES} +define+SOC_MMU_ENABLE=1 +incdir+"${ROOT_DIR}/rtl/include" \
  ...
  "${ROOT_DIR}/rtl/cpu/mips_tlb.v" "${ROOT_DIR}/rtl/cpu/mips_micro_tlb.v" "${ROOT_DIR}/rtl/cpu/mips_wb_stage.v" \
  "${ROOT_DIR}/tb/unit/mmu/tb_cpu_hardware_walker.sv" -top tb_cpu_hardware_walker -l compile.log
./simv -l sim.log
grep -q "REGRESSION_TEST_SUCCESS cpu_hardware_walker" sim.log
echo "CPU hardware walker integration: PASS"
