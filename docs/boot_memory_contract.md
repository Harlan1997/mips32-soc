...
- `soc_top.v` now exposes `qspi_io[3:0]` behind `ENABLE_QSPI_QUAD`, with
  ...
  replace this vendor-neutral boundary with the selected QSPI PHY/pad ring and
  validate qspi_sck/chip-select timing for the board;
...
3. Keep the standalone QSPI command/XIP bridge block-verified, while the
   ...
   `qspi_io[3:0]` boundary are now integrated and gated; next add four-lane
   command/AXI behavior, real PHY and production boot gates.
