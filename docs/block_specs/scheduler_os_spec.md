...
状态：v0.3，当前 RTL/仿真阶段 CONTRACT_CLOSED；scheduler 已以 opt-in 方式接入
...
- 上下文包含 PC、SP、status、ASID 和 32 个 GPR，由 scheduler 为每个 task 保存一份；`switch_valid` 表示恢复确认完成。
...
- `make cpu-scheduler-integration-gate` 已验证真实 `mips_cpu` 保存任务 0 并恢复
  任务 1 的 PC、GPR、GPR29/SP 和 CP0 ASID/Status。
