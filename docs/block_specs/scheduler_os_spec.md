...
- 切换期间 `scheduler_busy=1`，新的 timer/IPI/yield 触发不打断当前切换；
  scheduler 会保留一个 pending 请求，并在当前 restore 完成后按新的
  `active_mask` 发起一次后续切换。多个忙期间触发合并为一个请求，不要求软件
  通过重试循环补偿硬件丢失事件。
...
- `resched_ack` 与保存完成同周期有效，表示当前切换的保存阶段已被接收；
  忙期间触发由 pending 请求在后续 `ST_RUN` 周期消费。
