...
| RTL/QEMU Linux kernel retire prefix | PASS (bounded) | `build/isa_ref/qemu_linux_differential/completion_report.md`, `qemu/trace_compare.log` (`TRACE_COMPARE_PASS records=48781`) | Fresh relocated Linux vmlinux/DTB passes QEMU userspace boot and compares 48,781 aligned kernel retire records against the RTL boot-ROM run. The RTL run is cycle-bounded and still produces no userspace marker; full Linux system-mode differential remains open |
...
| Opt-in COP1 MTHC1/MFHC1 high-word differential | PASS | `SKIP_COVERAGE=1 VCS_JOBS=1 QEMU_BUILD_JOBS=1 EDA_MEMORY_MAX=1500M EDA_SWAP_MAX=512M BUILD_DIR=/tmp/fpu-highword-qemu-20260903 make qemu-system-fpu-single-differential-gate` | Legal high-word COP1 transfers compare through the RTL/QEMU retire boundary, including preservation of the low word in the even/odd double-pair representation. Complete FPE/IEEE-754, OS context/ABI and full COP1 compliance remain open. |
