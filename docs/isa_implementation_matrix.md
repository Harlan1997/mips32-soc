...
| SPECIAL3 EXT/INS, BSHFL/ALIGN/BITSWAP, RDHWR | implemented | `make isa-r2-gate mips-control-special3-gate bitswap-gate cp0-rdhwr-gate`; bounded R2 evidence: EXT/INS, all four `ALIGN` byte positions, RDHWR, BITSWAP and the other contract BSHFL operations have directed behavior/encoding coverage, including reserved `sa` values and fixed-`rs` checks. RDHWR `$0/$1/$2/$3/$29` has kernel and user-mode HWREna negative/positive coverage plus UserLocal/TLS runtime evidence. Full ISA compliance remains deferred |
