...
| P0 | QSPI standalone AXI/XIP bridge 已完成 block contract，但尚未进入 SoC memory subsystem；APB command 与 AXI/XIP 共享 SPI pins 的仲裁、优先级、abort、timeout 和 reset-in-flight 语义未定义 | 当前只能声明 `BLOCK_VERIFIED (vendor-neutral)`，不能把 bridge 当作 `SOC_INTEGRATED` 或产品 boot source；默认 `axi_spi_flash` 路径保持不变 | 先冻结共享 pin arbitration/timeout contract，再做参数化 SoC 接入、quad pad mux 和 no-preload boot gate；接入前不得升级状态 |
