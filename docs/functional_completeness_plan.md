...
| `integration/function-contract` | 唯一功能集成线；当前 HEAD `417a73b`，以 C2 `fcfc9c1` 为父线，已合入 C1 4-way I-cache、boot/memory 产品契约、Boot ROM/CP0 向量切片、kseg0 runtime ABI gate 和 UART 外部 RX waveform gate；worktree clean | 当前验证和后续产品功能变更只在此线收敛，暂不直接推入 `master` |
