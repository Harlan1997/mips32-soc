...
| FPU configurable rounding system differential | `make fpu-rounding-gate qemu-system-fpu-rounding-differential-gate` | PASS：RTL gate；`QEMU system RTL retire differential: PASS` | `CVT.W.S` 的四种 FCSR.RM 和固定 `ROUND.W.S` ties-away-from-zero 已通过真实 RTL 与 custom `mips32-soc-ref` 逐条 retire 比较。该项只闭合选定 rounding contract；完整 IEEE-754、FPE、Linux FPU ABI 和 full COP1 仍未闭合。 |
