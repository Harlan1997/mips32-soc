...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。产品 XIP read 以默认 512-cycle AXI guard 限时，uncached 非 OKAY response 经 CPU 转 IBE/DBE，cached refill/writeback 错误经 CacheErr sideband 转 ExcCode=30；开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1。新增 `qspi_cmd_behavioral` 保持独立，不接入 `soc_top`/AXI XIP/真实 PHY | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests、AXI timeout guard、I/D cache error unit、CPU CacheErr/ERL directed gate、`product-cacheerr-gate` 以及有效、11 条 header/CRC failure 和 timeout-to-DBE manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过；`qspi-cmd-behavioral-gate` 覆盖 APB LUT、status RX、RX/TX FIFO、24-bit address、x4 data、CS/SCLK、busy error、IRQ W1C 和 soft reset；统一 RTL frontend `3/3` 通过 | 单线 XIP 仍是 development reset-to-handoff；QSPI command/FIFO/四线当前为 `BLOCK_VERIFIED (vendor-neutral)`，不等于 `SOC_INTEGRATED`。仍无 production ROM/signature、ECC/完整软件错误分类、原始 SPI 无响应检测、AXI/XIP 接入、真实 flash model、quad pad wrapper、erase/program 生产路径、DDR init 或 U-Boot boot，仍为 P0 blocker |
...
- `qspi_cmd_behavioral` 已完成 vendor-neutral command/FIFO/quad block slice，但尚未接入 `soc_top` 或 AXI XIP；下一阶段仍需实现实际 QSPI boot source（AXI/XIP command path、image format、boot ROM）并完成 reset 到 first-stage firmware 的 SoC gate。
...
| P1 | 完成 QSPI 从 block contract 到 SoC/boot 集成，并补齐 UART pad/driver path 和 boot failure 分类 | QSPI command/FIFO/24-bit address/x4 data 已达到 `BLOCK_VERIFIED (vendor-neutral)`；仍未接入 `soc_top`/AXI XIP/真实 PHY；UART RTL pins、IRQ wiring、外部 RX waveform 和 SoC RX/PIC/RBR behavioral gate 已通过，pad-mux/板级/量产 RX driver 仍缺 | 当前 QSPI unit + RTL frontend PASS；后续需 AXI/XIP、真实 flash model、quad pad wrapper、erase/program/boot SoC gate；UART 需 pad/driver/板级 gate |
