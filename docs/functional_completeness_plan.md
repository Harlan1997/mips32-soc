...
- `SOC_MMU_ENABLE=1`：最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified policy gate、软件页表/context-switch 子集、4-ASID process-pressure、EBase `Mod` handler relocation/retry gate、stage-1 kseg0 指令交接和 20-word/stack runtime-depth gate，以及真实 cached-refill CacheErr handler/recovery gate 已通过；继续完成完整 kseg0 runtime linker/layout、cache maintenance、SoC page-table allocator/多进程压力、ECC/外部 EIC policy 和 kernel-mode firmware gate。refill/invalid 的 EBase/BEV 向量路由、CacheErr hardware vector 及 IP-based vectored interrupt 已有 directed 证据。
...
| P0 | 补齐 CPU/MMU 前端功能缺口：完整 kseg0 runtime/cache maintenance、SoC page-table/ASID allocator 与多进程调度/shootdown 压力、ECC/外部 EIC policy | 最小 refill/invalid/Modified/vector、CacheErr hardware contract + cached-refill recovery、软件/SoC page-table/context-switch 子集、4-ASID pressure、20-word/stack kseg0 depth slice 和硬件 index replacement 子集已有证据 | firmware + SoC simulation 通过，并记录异常/恢复矩阵；完整 linker/layout、`CACHE` 语义和 kernel 仍未关闭 |
