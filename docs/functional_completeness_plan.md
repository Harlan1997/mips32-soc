...
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified gate、软件 page-table/context-switch 子集、SoC ASID 1/2 与 TLBWI shootdown slice、SRAM EBase `Mod` recovery、IP-based vectored interrupt、MMU-enabled kseg0 instruction/单次 data slice、硬件 ASID index replacement slice 和 CacheErr hardware vector/ERL/ErrorEPC gate 已通过；完整 SoC page-table allocator、multi-process/scheduler 压力、TLB shootdown IPI、production cache-error handler/EIC policy 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动、4KB ASID/异常分类、软件/SoC context-switch 子集、`Mod` recovery、CacheErr hardware contract、CPU vector table、有限 kseg0 handoff 和 rollover boundary 证据，但不能作为可启动的产品 OS 功能 | 建立完整 kseg0 runtime linker/data layout、SoC page-table/ASID allocator 与多进程调度/shootdown 压力、production exception/cache-error handler policy 和 kernel firmware gate，再跑 exception regression。 |
...
| P0 | 补齐 CPU/MMU 前端功能缺口：完整 kseg0 runtime、SoC page-table/ASID allocator 与多进程调度/shootdown 压力、production cache-error handler/EIC policy | 最小 refill/invalid/Modified/vector、CacheErr hardware contract、软件/SoC page-table/context-switch 子集、单次 kseg0 data 和硬件 index replacement 子集已有证据 | firmware + SoC simulation 通过，并记录异常/恢复矩阵 |
