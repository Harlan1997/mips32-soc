...
| `integration/function-contract` | 唯一功能集成线；已合入 C1 4-way I-cache、boot/memory 产品契约、Boot ROM/CP0 向量切片、kseg0 runtime ABI gate、UART 外部 RX waveform gate 和 SoC RX integration gate；QSPI command/APB、vendor-neutral flash、quad pad 和 standalone AXI/XIP bridge 在本轮形成独立变更集 | 当前验证和后续产品功能变更只在此线收敛，暂不直接推入 `master`；AXI/XIP bridge 尚未切换默认 SoC memory path |
...
| XIP controller stall / bus error | 产品 `axi_spi_flash` 的 read AXI 通道由 `axi_read_timeout_guard` 包装，默认 512 cycles 内必须完成下游 `ARREADY` 和每个下一拍 `RVALID`；超时返回 `SLVERR`、延迟 response drain 后恢复。uncached response 透传为 IBE/DBE；cached refill/writeback 通过独立 sideband 透传为 CacheErr (ExcCode=30)；Boot ROM 当前仍记录 DBE/IBE `DEAD_B007`；APB `0x4000_5000` 提供版本、controller-present、timeout sticky 和最后错误码，并支持 W1C。另有 standalone `qspi_axi_xip` 通过 APB sequencer 对 vendor-neutral command engine 发起 x1 read | guard unit 覆盖 AR/R timeout、late drain、恢复和 sticky；qspi status integration gate 覆盖 guard-to-APB decode、错误分类和 W1C；完整产品 manifest gate 以 4-cycle guard 验证无 internal force/MISO 篡改的 DBE 到 `DEAD_B007`；I/D cache error unit 与 CPU CacheErr/ERL directed gate 均通过；`qspi-axi-xip-gate` 覆盖 AXI 单拍/两拍 burst、ID/RLAST/RRESP、command sequencing、flash readback 和 write rejection | `SOC_INTEGRATED` 的故障观测切片与 `BLOCK_VERIFIED (vendor-neutral)` 的 standalone AXI/XIP bridge 均有证据；但 bridge 未接 `soc_memory_subsystem`，APB command 与 AXI/XIP 的共享 pin 仲裁、timeout/abort、生产 handler/recovery、原始 SPI 静默 MISO 检测、quad pad/PHY 和 production erase/program 仍未实现 |
...
| D-cache NB WIP：`feature/dcache-nb-stage3@fcfc9c1` | 主工作区 `/home/admin/mips32-soc` 当前挂载此分支，仍有未提交 `rtl/cache/dcache_nb.v`、`tb/unit/dcache/tb_dcache_nb.v`、`tb/unit/run_dut_block_unit_gate.sh`、3 份 spec/roadmap 与 3 份 coverage 工件 | 已有唯一 feature 分支，但仍仅为 `BLOCK_VERIFIED`；未接入 CPU/SoC，不能计入 SoC 功能完成；不得从主工作区直接合并或计入集成线 |
...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。产品 XIP read 以默认 512-cycle AXI guard 限时，uncached 非 OKAY response 经 CPU 转 IBE/DBE，cached refill/writeback 错误经 CacheErr sideband 转 ExcCode=30；开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1。`qspi_apb_integration` 已接入外设 APB，保留旧 status map，并以 `+0x20` 暴露 command/FIFO；command CS 有效时由 SoC mux 接管共享单线 SPI pins；`spi_flash_behavioral` 可作为当前 command slice 的仿真 SPI 端点；`qspi_pad_wrapper` 独立提供四线三态 pad boundary；`qspi_axi_xip` standalone 通过 APB command path 提供 x1 AXI read-only bridge | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests、AXI timeout guard、I/D cache error unit、CPU CacheErr/ERL directed gate、`product-cacheerr-gate` 以及有效、11 条 header/CRC failure 和 timeout-to-DBE manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过；`qspi-cmd-behavioral-gate` 覆盖 APB LUT、status RX、RX/TX FIFO、24-bit address、x4 data、CS/SCLK、busy error、IRQ W1C 和 soft reset；扩展 `qspi-status-integration` 经 AXI→APB 配置/触发 `0x06` 并在共享 pins 捕获 8 bit、done/W1C；`qspi-flash-behavioral-gate` 经 `0x03` 读、`0x05` WEL、`0x06` WREN、`0x02` NOR page-program、`0x20` sector erase 和再次读回；`qspi-pad-wrapper-gate` 验证 x4 read/write pad direction/nibble 映射和高阻；`qspi-axi-xip-gate` 验证 AXI 单拍/两拍 burst、ID/RLAST/RRESP、APB sequencer、flash readback、AXI write `SLVERR` 和 pins idle；SoC smoke 与统一 RTL frontend `3/3` 通过 | 已达到有限 `SOC_INTEGRATED` APB/x1 command slice，且有 vendor-neutral flash protocol endpoint、standalone tri-state pad boundary 和 standalone AXI/XIP bridge 证据，但不是完整 QSPI 产品路径。仍无 SoC memory subsystem AXI/XIP integration、APB-vs-XIP 共享 pin 仲裁/abort/timeout contract、商用 flash 许可模型/板级模型、SoC 四线 pad mux/真实 PHY、erase/program production path、production ROM/signature、ECC/完整软件错误分类、原始 SPI 无响应检测、DDR init 或 U-Boot boot，仍为 P0 blocker |
...
- `qspi_cmd_behavioral` 已通过 block gate，并由 `qspi_apb_integration` 接入 SoC APB/共享单线 pins；当前只关闭 command-only/x1 slice。下一阶段仍需实现实际 QSPI boot source（AXI/XIP command path、image format、quad pad/PHY、boot ROM）并完成 reset 到 first-stage firmware 的 SoC gate。
...
- 明确当前限制：1 MSHR、2-slot order queue、下游单 outstanding；2+ MSHR、store buffer 和完整 kernel cache contract 继续列为 backlog。CACHE 六种 D-cache maintenance operation、I/D-cache 有限 TagLo 读写、TagHi 读写和 SYNC ordered-no-op 已有独立 CPU/block/集成证据；parity/ECC、复杂 outstanding/store-buffer 的 SYNC 语义和生产 OS ABI 仍未闭合。
...
| 2026-08-02 | `integration/function-contract` branch/WIP audit | `git worktree list`；`git branch -avv`；`git -C /home/admin/mips32-soc status --short` | PASS：功能线本轮起始 `integration/function-contract@b77898e` clean；`master@6ecbbbc` 保持基线；主工作区挂载 `feature/dcache-nb-stage3@fcfc9c1` 且存在未提交 D-cache NB RTL/TB、spec/roadmap 和 coverage 工件 | D-cache NB WIP 继续隔离，不计入当前 RTL 功能完成；任何后续合并必须先在该分支形成独立 commit、block gate 和 owner，再由功能线审查合入。 |
...
| P0 | UART RTL pins/IRQ wiring、外部 RX waveform 和 SoC RX/PIC/RBR behavioral 路径已有证据，但 pad-mux/板级电气绑定、量产 RX driver 仍未验收；WDT APB/reset pulse、boot-status retention、预加载 firmware retention 和无 SRAM preload Boot ROM failure slice 已通过 | 对外 serial I/O 仍缺板级证据；WDT gate 只证明 deliberate Boot ROM failure/reset，不证明 manifest/QSPI/DDR 真实故障分类、量产 ROM 和板级 reset 观测 | 固化 UART pad contract、板级 gate、flow-control 实线和端到端量产 RX driver；把真实 manifest/controller/DDR failure 映射到 failure code，并增加板级 watchdog/reset 观测。 |
...
| P0 | QSPI standalone AXI/XIP bridge 和 shared-pin arbiter contract 已完成 block gate，但尚未进入 SoC memory subsystem；command trigger/AXI `AR` acceptance 与 request/grant 的绑定、abort、timeout 和 reset-in-flight 语义仍未定义 | 当前只能声明 `BLOCK_VERIFIED (vendor-neutral)`，不能把 bridge 或 arbiter 当作 `SOC_INTEGRATED` 或产品 boot source；默认 `axi_spi_flash` 路径保持不变 | 将 APB command trigger 与 AXI acceptance 接入 arbiter request/grant，先做冲突/释放/错误响应 SoC gate，再做 quad pad mux 和 no-preload boot gate；接入前不得升级状态 |
...
| P1 | 完成 QSPI 从有限 SoC command slice 和 standalone AXI/XIP bridge 到完整 SoC/boot 集成，并补齐 UART pad/driver path 和 boot failure 分类 | QSPI command/FIFO/24-bit address/x4 data 与 standalone AXI/XIP bridge 为 `BLOCK_VERIFIED (vendor-neutral)`；APB/x1 command 已达到有限 `SOC_INTEGRATED`，vendor-neutral SPI flash endpoint 和 standalone quad pad boundary gate 已通过，但 `soc_memory_subsystem` AXI/XIP、共享 pin 仲裁/timeout/abort、SoC 四线 mux/真实 PHY/商用 flash/boot 未闭合；UART RTL pins、IRQ wiring、外部 RX waveform 和 SoC RX/PIC/RBR behavioral gate 已通过，pad-mux/板级/量产 RX driver 仍缺 | 当前 QSPI block + APB/SoC x1 + standalone AXI/XIP + SPI flash behavioral + quad pad wrapper + RTL frontend + SoC smoke PASS；后续需冻结共享 pin contract，完成 SoC AXI/XIP 接入、商用 flash model/板级模型、SoC quad pad mux/PHY、production erase/program/boot SoC gate；UART 需 pad/driver/板级 gate |
