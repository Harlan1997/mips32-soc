...
| XIP controller stall / bus error | 产品 `axi_spi_flash` 的 read AXI 通道由 `axi_read_timeout_guard` 包装，默认 512 cycles 内必须完成下游 `ARREADY` 和每个下一拍 `RVALID`；超时返回 `SLVERR`、延迟 response drain 后恢复。I-cache/D-cache 非 OKAY response 透传为 CPU IBE/DBE，Boot ROM 的 DBE/IBE handler 写 `DEAD_B007`；APB `0x4000_5000` 提供版本、controller-present、timeout sticky 和最后错误码，并支持 W1C | guard unit 覆盖 AR/R timeout、late drain、恢复和 sticky；qspi status integration gate 覆盖 guard-to-APB decode、错误分类和 W1C；完整产品 manifest gate 以 4-cycle guard 验证无 internal force/MISO 篡改的 DBE 到 `DEAD_B007`；I/D cache error unit 均通过 | `SOC_INTEGRATED` 的故障观测切片：保护 AXI/controller 挂死并提供软件可读状态，但不可检测无 ready/error 的原始 SPI 静默 MISO；完整 cache-error vector policy、QSPI command/FIFO/erase/program/四线仍未实现 |
...
| 优先级 | 问题 | 对计划的影响 | 处理条件 |
|---|---|---|---|
| P0 | 产品 boot、DDR 和 QSPI 尚未闭合：Boot ROM 复位、普通与 refill/invalid BEV-EBase vector、IP-based vectored interrupt、最小 BEV MMU firmware、单一 EBase `Mod` recovery、development manifest header/CRC-to-SRAM handoff，以及 controller/AXI stall-to-DBE 和 QSPI timeout status 观测已通过；ASIC Profile C1 DDR4 已选但 `DDR4-IN-01..08` 仍未登记，旧 `DDR-IN-01..08` 仅为 legacy DDR3 边界，`ddr-contract-entry-audit` 已确认契约一致但 `DDR4_ENTRY_READY=0`；PHY/IP、DRAM part/timing file、real memory model、WDT budget 和 RTL 仍未实现；生产 ROM/signature、原始 SPI 无响应检测、cache-error/EIC policy、QSPI/U-Boot/Linux 也未闭合 | SoC 已有受限的 reset-to-development-stage-1、behavioral DDR store/load、XIP transport-stall failure 和软件可读 timeout evidence，但无可发布的 secure boot 或产品主存，不能称商用 SoC | 先完成 `docs/ddr4_integration_inputs.md` 的 `DDR4-IN-01..08` 输入登记并使 `DDR4_ENTRY_READY=1`，重建 DDR4 controller/PHY contract；之后实现真实 PHY/controller、APB status、init/training/refresh、bounded AXI error path 和 no-preload boot gate；并继续实现完整 runtime exception/cache-error policy、真正 QSPI command/FIFO/四线控制器、boot-status/WDT 与 production handoff，分别验证。 |
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified gate、SRAM EBase `Mod` recovery、IP-based vectored interrupt、MMU-enabled kseg0 instruction/单次 data slice 和硬件 ASID index replacement slice 已通过；page-table walk/OS ASID allocator、cache-error/EIC policy 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动、4KB ASID/异常分类、`Mod` recovery、CPU vector table、有限 kseg0 handoff 和 rollover boundary 证据，但不能作为可启动的产品 OS 功能 | 建立完整 kseg0 runtime linker/data layout、page-table walk、OS ASID rollover/context-switch、exception policy 和 kernel firmware gate，再跑 exception regression。 |
...
| P0 | 补齐 CPU/MMU 前端功能缺口：完整 kseg0 runtime、page-table walk/OS ASID rollover、cache-error/EIC policy | 最小 refill/invalid/Modified/vector、单次 kseg0 data 和硬件 index replacement 子集已有证据 | firmware + SoC simulation 通过，并记录异常/恢复矩阵 |
