...
| MMU/TLB | RTL 与 unit TB 存在，默认关闭；产品 opt-in 具备 CacheErr、refill/invalid vector routing、最小 Boot ROM kseg1 linker、BEV refill handler、wired kseg2-APB map，以及复制到 SRAM 的 EBase `Mod` handler | MMU/CP0 unit、`make tlb-asid-policy-gate`、`make cpu-cache-error-gate`、完整 SoC I/D vector directed、`make product-mmu-boot-gate` 和 `make product-mmu-ebase-modified-gate` 通过；ASID gate 覆盖 4KB 非 Global 隔离、Global 跨 ASID、matching-invalid 的 TLBL/TLBS 和 clean-store 的 Modified；后者覆盖 DTLB `Mod`、CP0 precise state、EBase handler relocation、`D` bit repair 和 `ERET` retry；IP-based vectored interrupt 有独立 SoC gate | 最小 BEV 启动、4KB ASID/异常分类、CacheErr hardware contract、单一 EBase `Mod` recovery 与 CPU vector table 已有证据；完整 kseg0 runtime、page-table/ASID rollover、生产 cache-error handler、kernel/OS boot 仍未闭合 |
...
当前基线 `40a360d` 已满足 `RTL_FRONTEND_COMPILE_READY`，并已具备
...
- `SOC_MMU_ENABLE=1`：最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified policy gate、EBase `Mod` handler relocation/retry gate，以及 stage-1 kseg0 指令交接 gate 已通过；继续完成完整 kseg0 runtime 数据路径、page-table/ASID rollover、生产 cache-error handler/EIC 和 kernel-mode firmware gate。refill/invalid 的 EBase/BEV 向量路由、CacheErr hardware vector 及 IP-based vectored interrupt 已有 directed 证据。
...
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified gate、SRAM EBase `Mod` recovery、IP-based vectored interrupt、MMU-enabled kseg0 instruction/单次 data slice、硬件 ASID index replacement slice 和 CacheErr hardware vector/ERL/ErrorEPC gate 已通过；page-table walk/OS ASID allocator、production cache-error handler/EIC policy 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动、4KB ASID/异常分类、`Mod` recovery、CacheErr hardware contract、CPU vector table、有限 kseg0 handoff 和 rollover boundary 证据，但不能作为可启动的产品 OS 功能 | 建立完整 kseg0 runtime linker/data layout、page-table walk、OS ASID rollover/context-switch、production exception/cache-error handler policy 和 kernel firmware gate，再跑 exception regression。 |
...
| P0 | 补齐 CPU/MMU 前端功能缺口：完整 kseg0 runtime、page-table walk/OS ASID rollover、production cache-error handler/EIC policy | 最小 refill/invalid/Modified/vector、CacheErr hardware contract、单次 kseg0 data 和硬件 index replacement 子集已有证据 | firmware + SoC simulation 通过，并记录异常/恢复矩阵 |
