...
现有四条真实 CPU workload 现在额外报告固定点 `CPI x1000` 和 `IPC x1000`，
直接由 cycle/retire APB counter delta 计算，不引入 freestanding FPU 或
...
| 2026-08-23 | `integration/function-contract` CPU performance workload observation slice | `make perf-workloads-gate`；`build/soc_test/perf_workloads/sim.log` | PASS：真实 CPU/APB `SOC_PERF_COUNTERS=1`；四条 sequential/strided/branch-mixed/MDU workload 均输出 cycle、retire、固定点 CPI/IPC、I/D miss、branch 和 MDU delta，版本 `0x50430001`，并达到 `REGRESSION_TEST_SUCCESS` | 这是可重复的微架构观测基线，不是官方 CoreMark/Dhrystone/STREAM 成绩或商用性能 signoff；标准 benchmark 源码/性能目标仍 deferred。 |
...
| 2026-09-01 | `integration/function-contract` MMU allocator atomic lease handoff | `make mmu-page-table-allocator-gate tlb-asid-allocator-gate mmu-context-contract-gate rtl-frontend-compile BUILD_DIR=/tmp/mips32-allocator-atomic` | PASS：root/ASID allocator 均验证满池同周期 release+alloc、递增 generation token；frontend `8/8` | 修复页表根和 ASID lease allocator 在 owner 释放与新 owner 分配同周期时错误报告耗尽的问题；旧 generation 仍被拒绝。该改动只闭合 bounded ownership 生命周期，不扩展为 Linux page-table population、CPU demand paging、scheduler/多核 shootdown 或 full MMU compliance。 |
...
| CPU/MMU ASID allocator | `make tlb-asid-allocator-gate`、`make mmu-context-contract-gate`、`make product-mmu-context-cpu-gate` | PASS | 四槽分配/耗尽、stale generation 拒绝、释放后 generation 递增并复用，以及满池同周期 release+alloc 的 generation-safe 原子交接均已验证；完整 page-table walker/OS allocator 仍不在当前 RTL contract |
...
| QEMU FPU W-conversion indefinite boundary | `make fpu-single-gate qemu-system-fpu-single-differential-gate` | PASS：RTL `FPU PASS`；QEMU system-mode direct run `FPU PASS`；`TRACE_COMPARE_PASS records=1320` | 项目自有 QEMU 9.2 patch 将 invalid/overflow 的 `CVT/ROUND/TRUNC/CEIL/FLOOR.W.{S,D}` 结果统一为 MIPS indefinite `0x80000000`，并纳入 custom-machine 构建输入 hash；完整 IEEE-754 和 FPU OS/ABI 仍未闭合。 |
| Legacy MIPS SNaN/QNaN compare classification | `make mips-fpu-compare-gate fpu-single-gate qemu-system-fpu-single-differential-gate rtl-frontend-compile` | PASS：primitive `REGRESSION_TEST_SUCCESS`；SoC `FPU PASS`；QEMU `TRACE_COMPARE_PASS records=1320`；frontend `8/8` | 24Kf pre-NAN2008 编码下，fraction quiet bit 为 1 的 signaling NaN 对全部 16 个 C.* predicate 置 Invalid；quiet NaN 对 predicates 0..7 不置 Invalid、对 8..15 置 Invalid。新增 single/double vectors 并保持既有 QEMU differential corpus 一致；完整 IEEE-754 NAN2008 policy、FPE delivery、OS FPU ABI 和 full COP1 compliance 仍未闭合。 |
| QEMU FPU double `SQRT.D` differential slice | `make fpu-double-gate`、`make qemu-system-fpu-double-differential-gate` | PASS：RTL `FPU PASS`；`TRACE_COMPARE_PASS records=297` | 在既有 double pair/COP1X corpus 中加入真实 `SQRT.D $f2,$f0`（`4.0D -> 2.0D`）结果检查，RTL 与 `mips32-soc-ref` 通过逐条 retire 比较；这只是选定指令 slice，不代表完整 IEEE-754、FPE、OS FPU context/ABI 或 full COP1 compliance。 |
| FPU configurable rounding system differential | `make fpu-rounding-gate qemu-system-fpu-rounding-differential-gate` | PASS：RTL gate；`QEMU system RTL retire differential: PASS` | `CVT.W.S/CVT.W.D` 的四种 FCSR.RM 和固定 `ROUND.W.S/ROUND.W.D` ties-away-from-zero 已通过真实 RTL 与 custom `mips32-soc-ref` 逐条 retire 比较。该项只闭合选定 rounding contract；完整 IEEE-754、FPE、Linux FPU ABI 和 full COP1 仍未闭合。 |
| RTL/QEMU Linux kernel retire prefix | `make qemu-system-linux-differential-gate` | PASS：`TRACE_COMPARE_PASS records=48781`；QEMU Linux userspace gate PASS | 修正 Linux differential runner 的 stale handoff PC，并按 handoff 对齐后的 RTL 长度截取 QEMU golden；同一 relocated kernel/DTB 完成 48,781 条 kernel retire 严格比较。RTL 仍为 100k-cycle bounded run、无 userspace marker，因此 full Linux system-mode differential、OS VM ownership 和完整 ISA/privileged signoff 仍未闭合。 |
...
### 2026-09-02 MMU eight-by-eight deterministic pressure extension

The `mmu_process_pressure` corpus now touches eight 4-KiB demand pages for
each of eight software ASIDs, covering 32 dynamic TLB pairs. The refill
handler allocates non-overlapping eight-page PFN regions, the reverse context
pass checks isolation, and the post-shootdown pass forces the mappings to
demand-refill again. The real RTL gate passes with `refills=73`, and the
strict QEMU system differential passes using a software-owned round-robin
`TLBWI` index (1..63). `TLBWR` is deliberately excluded from this differential
corpus because its hardware-random timing is not a frozen RTL/QEMU contract.
This advances bounded demand paging coverage only; unrestricted Linux
...
The FPE boundary slices are now connected to strict RTL/QEMU retire
differential evidence. The combined
`qemu-system-fpu-fpe-boundary-differential-gate` runs the selected single
Inexact/Invalid/Overflow/Underflow cases, the double divide-by-zero/Invalid/
Overflow three-class case, and the double Inexact and Underflow cases. The
nine selected boundary classes pass against `mips32-soc-ref`, including
`CVT.W.D 1.5` with Inexact enabled and the minimum-positive-double-subnormal
times `0.5` Underflow case. This does not close complete IEEE-754
range/tininess behavior, full double FPE coverage, lazy-FPU/signal-frame ABI,
or full COP1 compliance.
