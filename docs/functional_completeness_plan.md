...
| Development manifest handoff | Boot ROM 经实际单线 SPI XIP 读取固定 64-byte `SOC1` development manifest，CRC32 校验后把 stage-1 拷贝至 Boot SRAM 并跳转 `0x8000_1000`；同时修复 SPI 读相位和 MMU C=2 D-cache 属性路由 | 完整 SoC directed gate 覆盖有效镜像和 bad-CRC 镜像，且不预加载 SRAM、不使用 `axi_flash_image_model`；可由独立 Make target 和 block aggregate 重跑 | 仅为 `BLOCK_VERIFIED` 的 development boot 子集。没有签名、bad-magic/bounds/controller-timeout negative、QSPI 写擦/四线、DDR handoff、boot-status/WDT 或生产 ROM artifact |
...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1 | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests，以及有效/bad-CRC manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过 | 真实单线 XIP 具备 development reset-to-handoff 证据；无 production ROM/signature、完整 manifest negative、QSPI command/FIFO/erase/program/四线、DDR init 或 U-Boot boot，仍为 P0 blocker |
...
历史 full signoff 的功能阶段均通过，coverage 阈值单独失败，保留为后续质量工作。**Phase 0 的 C1/C2 分支整理、Phase 2 boot/memory 架构冻结、Boot ROM 复位/普通向量、TLB refill/invalid 的 BEV-EBase 路由、最小 Boot ROM kseg1 linker/BEV refill/wired mapping、复制到 SRAM 的 EBase `Mod` recovery，以及经实际 SPI XIP 的 development manifest/CRC-to-kseg0-SRAM handoff 已完成。当前下一项是扩展 manifest 的 bad-magic/bounds/controller-error/timeout 负向路径，并行定义完整 kseg0 runtime linker、invalid/ASID policy、vectored/cache-error policy；仍不是 coverage closure。**
...
| 优先级 | 问题 | 对计划的影响 | 处理条件 |
|---|---|---|---|
| P0 | 产品 boot、DDR 和 QSPI 尚未闭合：Boot ROM 复位、普通与 refill/invalid BEV-EBase vector、最小 BEV MMU firmware、单一 EBase `Mod` recovery，以及 development manifest CRC-to-SRAM handoff 已通过；生产 ROM/signature、其余 manifest failure、剩余 vector policy、QSPI/U-Boot/Linux、DDR controller/PHY 仍未实现 | SoC 已有受限的 reset-to-development-stage-1 证据，但无可发布的 secure boot 或产品主存，不能称商用 SoC | 在本文件 Phase 2 继续实现 manifest 负向矩阵、完整 runtime exception policy、QSPI、DDR、boot-status/WDT 与 production handoff，并分别验证。 |
