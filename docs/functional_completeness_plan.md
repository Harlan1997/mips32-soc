...
- 4-way I-cache 已合入并完成通用 unit/SoC 证据；`mips_core` CPU/AXI execution gate 覆盖 reset 后真实取指、六个同 set tag 的 line refill、四路容量压力和后续 eviction/refill，error gate 覆盖指定 refill 的 `SLVERR` -> I-cache CacheErr -> CP0 `ExcCode=30/ERL`，产品 error gate 再覆盖 `BFC0_0000`/`1FC0_0000` 首笔失败、`BFC0_0100`/`1FC0_0100` vector、精确 ErrorEPC、ERET 和重取；仍需长期压力 sequence 后，才能将其标为 `CONTRACT_CLOSED`。
...
- 明确当前限制：1 MSHR、2-slot order queue、下游单 outstanding；2+ MSHR、store buffer 和完整 kernel cache contract 继续列为 backlog。CACHE 六种 D-cache maintenance operation、有限 TagLo/TagHi 读写和 SYNC ordered-no-op 已有独立 CPU/block 证据；I-cache TagLo/TagHi、parity/ECC、复杂 outstanding/store-buffer 的 SYNC 语义和生产 OS ABI 仍未闭合。

### Phase 4：外设与系统软件功能闭合

- `SOC_MMU_ENABLE=1`：最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified policy gate、软件页表/context-switch 子集、4-ASID process-pressure、EBase `Mod` handler relocation/retry gate、stage-1 kseg0 指令交接、20-word/stack kseg0 runtime-depth gate、`.rodata/.data/.bss/stack` runtime-layout slice，以及真实 cached-refill CacheErr handler/recovery gate 已通过；CACHE 六种 D-cache maintenance op、有限 TagLo/TagHi CP0 读写和 SYNC ordered-no-op 已有 CPU/块级证据；I-cache 已有 CPU/AXI execution/refill/eviction、普通 `SLVERR`/CacheErr/ERL 和产品启动 physical vector/ErrorEPC/ERET retry slice；继续完成完整 runtime linker/loader ABI、SoC page-table allocator/多进程压力、I-cache long-stress contract、ECC/外部 EIC policy 和 kernel-mode firmware gate。refill/invalid 的 EBase/BEV 向量路由、CacheErr hardware vector 及 IP-based vectored interrupt 已有 directed 证据。
...
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified gate、软件 page-table/context-switch 子集、SoC ASID 1/2 与 TLBWI shootdown slice、4-ASID process-pressure slice、SRAM EBase `Mod` recovery、IP-based vectored interrupt、MMU-enabled kseg0 instruction/单次 data slice、20-word/stack kseg0 runtime-depth 与 `.rodata/.data/.bss/stack` runtime-layout slice、硬件 ASID index replacement slice、CacheErr hardware vector/ERL/ErrorEPC 和注入式 handler/recovery gate、产品启动 I-cache CacheErr vector/ERET retry gate、CACHE maintenance/有限 TagLo/TagHi/SYNC slice 已通过；完整 SoC page-table allocator、multi-process/scheduler 长期压力、TLB shootdown IPI、I-cache 长期压力/tag ABI、ECC/外部 EIC policy 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动、4KB ASID/异常分类、软件/SoC context-switch 子集、bounded process pressure、`Mod` recovery、CacheErr hardware contract + recovery slice、CPU vector table、产品复位 I-cache 首笔错误恢复、有限 kseg0 handoff/runtime-depth/runtime-layout、CACHE D-cache maintenance 和有限 TagLo/TagHi/SYNC 证据，但不能作为可启动的产品 OS 功能 | 继续扩展完整 runtime ABI/loader（含可重定位数据、stack/heap、异常入口）、SoC page-table/ASID allocator 与多进程调度/shootdown 压力、I-cache 长期压力/tag ABI、ECC/production exception/cache-error policy 和 kernel firmware gate，再跑 exception regression。 |
...
| P0 | 补齐 CPU/MMU 前端功能缺口：完整 kseg0 runtime、SoC page-table/ASID allocator 与多进程调度/shootdown 压力、I-cache tag contract、ECC/外部 EIC policy | 最小 refill/invalid/Modified/vector、CacheErr hardware contract + cached-refill recovery、产品启动 I-cache 首笔错误/vector/ERET retry、软件/SoC page-table/context-switch 子集、4-ASID pressure、20-word/stack kseg0 depth slice、硬件 index replacement、CACHE 六种 D-cache maintenance operation、有限 TagLo/TagHi/SYNC 已有证据 | firmware + SoC simulation 通过，并记录异常/恢复矩阵；完整 linker/layout、I-cache 长期压力/tag ABI、复杂 ordering、ECC/production policy 和 kernel 仍未关闭 |
