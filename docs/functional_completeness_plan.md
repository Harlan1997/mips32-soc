...
| `integration/function-contract` | 唯一功能集成线；当前 HEAD `661fa82`，以 C2 `fcfc9c1` 为父线，已合入 C1 4-way I-cache、boot/memory 产品契约、Boot ROM/CP0 向量切片、kseg0 runtime ABI gate、UART 外部 RX waveform gate 和 SoC RX integration gate；worktree clean | 当前验证和后续产品功能变更只在此线收敛，暂不直接推入 `master` |
...
| P1 | 补齐 QSPI production command/FIFO/四线 RTL 契约、UART pad/driver path 和 boot failure 分类 | QSPI 仍只有 development SPI XIP；UART RTL pins、IRQ wiring、外部 RX waveform 和 SoC RX/PIC/RBR behavioral gate 已通过，pad-mux/板级/量产 RX driver 仍缺 | unit + firmware + SoC negative/reset simulation 通过 |
