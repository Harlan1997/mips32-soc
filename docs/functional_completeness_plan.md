...
| Development manifest handoff | Boot ROM 经实际单线 SPI XIP 读取固定 64-byte `SOC1` development manifest，CRC32 校验后把 stage-1 拷贝至 Boot SRAM 并跳转 `0x8000_1000`；MMU-enabled slice 额外确认 kseg0 入口取指 `0x8000_1000 -> 0x0000_1000`，并由 stage-1 发起 kseg0 数据访问 `0x8000_7000 -> 0x0000_7000`；同时修复 SPI 读相位、MMU C=2 D-cache 属性路由和长路径镜像截断 | 完整 SoC directed gate 覆盖有效镜像，及 11 条单字段 header/CRC 失败镜像；不预加载 SRAM、不使用 `axi_flash_image_model`，并拒绝未加载的全 `FF` fixture，可由独立 Make target 和 block aggregate 重跑 | 仍为 `BLOCK_VERIFIED` 的 development boot 与 kseg0 指令/单次数据交接子集。没有完整 runtime 数据布局、SoC page-table allocator/shootdown/ASID 压力、签名、QSPI 写擦/四线、DDR handoff、boot-status/WDT 或生产 ROM artifact |
...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。产品 XIP read 以默认 512-cycle AXI guard 限时，uncached 非 OKAY response 经 CPU 转 IBE/DBE，cached refill/writeback 错误经 CacheErr sideband 转 ExcCode=30；开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1 | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests、AXI timeout guard、I/D cache error unit、CPU CacheErr/ERL directed gate，以及有效、11 条 header/CRC failure 和 timeout-to-DBE manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过 | 真实单线 XIP 具备 development reset-to-handoff、字段拒绝、controller/AXI stall failure 和最小 CacheErr hardware evidence；无 production ROM/signature、CacheErr handler/recovery、原始 SPI 无响应检测、QSPI command/FIFO/erase/program/四线、软件错误状态、DDR init 或 U-Boot boot，仍为 P0 blocker |
| MMU/TLB | RTL 与 unit TB 存在，默认关闭；产品 opt-in 具备 CacheErr、refill/invalid vector routing、最小 Boot ROM kseg1 linker、BEV refill handler、wired kseg2-APB map，以及复制到 SRAM 的 EBase `Mod` handler | MMU/CP0 unit、`make tlb-asid-policy-gate`、`make tlb-os-context-gate`、`make cpu-cache-error-gate`、完整 SoC I/D vector directed、`make product-mmu-boot-gate` 和 `make product-mmu-ebase-modified-gate` 通过；ASID gate 覆盖 4KB 非 Global 隔离、Global 跨 ASID、matching-invalid 的 TLBL/TLBS 和 clean-store 的 Modified；OS-context gate 覆盖软件页表 walk、同 VA 不同 ASID/PFN、wired global 保留、非 wired flush 和 1..255 回卷；后者覆盖 DTLB `Mod`、CP0 precise state、EBase handler relocation、`D` bit repair 和 `ERET` retry；IP-based vectored interrupt 有独立 SoC gate | 最小 BEV 启动、4KB ASID/异常分类、软件管理 TLB context-switch slice、CacheErr hardware contract、单一 EBase `Mod` recovery 与 CPU vector table 已有证据；完整 kseg0 runtime、SoC firmware page-table allocator、生产 cache-error handler、kernel/OS boot 仍未闭合 |
...
- `SOC_MMU_ENABLE=1`：最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified policy gate、软件页表/context-switch 子集、EBase `Mod` handler relocation/retry gate，以及 stage-1 kseg0 指令交接 gate 已通过；继续完成完整 kseg0 runtime 数据路径、SoC page-table allocator/多进程压力、生产 cache-error handler/EIC 和 kernel-mode firmware gate。refill/invalid 的 EBase/BEV 向量路由、CacheErr hardware vector 及 IP-based vectored interrupt 已有 directed 证据。
...
rollover、production cache-error handler/EIC policy、QSPI production path 和真实 DDR4 product entry 仍未完成。**
...
| 2026-08-02 | `integration/function-contract` MMU-enabled kseg0 data slice | `make product-kseg0-runtime-gate PRODUCT_KSEG0_RUNTIME_DIR=build/unit_tb/product_kseg0_runtime_data`；`make product-manifest-handoff-gate PRODUCT_MANIFEST_HANDOFF_DIR=build/unit_tb/product_manifest_handoff_kseg0_data`；`make dut-block-unit-gate DUT_BLOCK_UNIT_DIR=build/unit_tb/rtl_frontend_block_aggregate` | PASS：MMU-enabled handoff 的 stage-1 kseg0 instruction/data checks、valid manifest、11 个 rejection、XIP timeout 均通过；Boot ROM/block aggregate `10/10` | 新增 `EXPECT_KSEG0_DATA` 检查 CPU data VA `0x8000_7000` 与 PA `0x0000_7000`，只证明一次 kseg0 data translation/cache request；不升级为完整 runtime data mapping 或 OS/page-table evidence。 |
...
| 2026-08-02 | `integration/function-contract` OS page-table/context-switch slice | `make tlb-os-context-gate TLB_OS_CONTEXT_DIR=build/unit_tb/tlb_os_context_try2` | PASS：`REGRESSION_TEST_SUCCESS tlb_os_context` | 以真实 `mips_tlb + mips_mmu` 建立软件页表 fixture；覆盖同一 VA 的 ASID 1/2 不同 PFN、VPN pair even/odd、wired global 映射、ASID 1..255 分配、非 wired flush 和回卷后重新填充。仍不是 SoC firmware、TLB shootdown/IPI、Linux page-table 或 demand paging 证据。 |
| 2026-08-02 | `integration/function-contract` CacheErr exception slice | `make cpu-cache-error-gate CPU_CACHE_ERROR_DIR=build/unit_tb/cpu_cache_error` | PASS：`REGRESSION_TEST_SUCCESS mips_cpu_cacheerr` | D-cache cached-error sideband 使用 ExcCode=30，产品模式跳转 `EBase+0x100`，CP0 置 `ERL=1`、保持 `EXL=0` 并保存精确 `ErrorEPC`；uncached DBE policy 未改变。该 gate 不代表 production cache-error handler/recovery、ECC、EIC/VEIC 或完整 cache maintenance。 |

## 10. 已知未决问题

| 优先级 | 问题 | 对计划的影响 | 处理条件 |
|---|---|---|---|
| P0 | 产品 boot、DDR 和 QSPI 尚未闭合：Boot ROM 复位、普通与 refill/invalid BEV-EBase vector、IP-based vectored interrupt、最小 BEV MMU firmware、单一 EBase `Mod` recovery、development manifest header/CRC-to-SRAM handoff，以及 controller/AXI stall-to-DBE 和 QSPI timeout status 观测已通过；ASIC Profile C1 DDR4 已选但 `DDR4-IN-01..08` 仍未登记，旧 `DDR-IN-01..08` 仅为 legacy DDR3 边界，`ddr-contract-entry-audit` 已确认契约一致但 `DDR4_ENTRY_READY=0`；PHY/IP、DRAM part/timing file、real memory model、WDT budget 和 RTL 仍未实现；生产 ROM/signature、原始 SPI 无响应检测、production cache-error handler/EIC policy、QSPI/U-Boot/Linux 也未闭合 | SoC 已有受限的 reset-to-development-stage-1、behavioral DDR store/load、XIP transport-stall failure、软件可读 timeout 和最小 CacheErr hardware evidence，但无可发布的 secure boot 或产品主存，不能称商用 SoC | 先完成 `docs/ddr4_integration_inputs.md` 的 `DDR4-IN-01..08` 输入登记并使 `DDR4_ENTRY_READY=1`，重建 DDR4 controller/PHY contract；之后实现真实 PHY/controller、APB status、init/training/refresh、bounded AXI error path 和 no-preload boot gate；并继续实现完整 runtime exception/cache-error handler、真正 QSPI command/FIFO/四线控制器、boot-status/WDT 与 production handoff，分别验证。 |
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified gate、软件 page-table/context-switch 子集、SRAM EBase `Mod` recovery、IP-based vectored interrupt、MMU-enabled kseg0 instruction/单次 data slice、硬件 ASID index replacement slice 和 CacheErr hardware vector/ERL/ErrorEPC gate 已通过；完整 SoC page-table allocator、TLB shootdown/多进程压力、production cache-error handler/EIC policy 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动、4KB ASID/异常分类、软件 context-switch 子集、`Mod` recovery、CacheErr hardware contract、CPU vector table、有限 kseg0 handoff 和 rollover boundary 证据，但不能作为可启动的产品 OS 功能 | 建立完整 kseg0 runtime linker/data layout、SoC page-table/ASID allocator 与 shootdown/context-switch 压力、production exception/cache-error handler policy 和 kernel firmware gate，再跑 exception regression。 |
...
| P1 | crossbar 支持 cross-slave 并发，但 L2/APB/flash/DDR slave 仍限制同 slave outstanding；SPI serial boot、production cache-error handler、EIC/VEIC policy、MMU/Linux boot 均未纳入当前 RTL contract | 即使当前功能 gate 通过，也只能声明当前已文档化契约，不是商用 SoC 完整性结论 | 每项建独立 spec/RTL/firmware/UVM 变更集，并按本计划的五级状态推进。 |
...
| P0 | 补齐 CPU/MMU 前端功能缺口：完整 kseg0 runtime、SoC page-table/ASID allocator 与 shootdown/context-switch 压力、production cache-error handler/EIC policy | 最小 refill/invalid/Modified/vector、CacheErr hardware contract、软件 page-table/context-switch 子集、单次 kseg0 data 和硬件 index replacement 子集已有证据 | firmware + SoC simulation 通过，并记录异常/恢复矩阵 |
