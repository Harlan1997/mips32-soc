...
当前产品路线已确定为 **ASIC Profile C**。这只关闭了工艺档位选择，没有关闭
内存代际和实现输入：
...
当前必须先在 C1 DDR4 和 C2 LPDDR4/4X 中二选一，再建立新的 memory contract；
因此状态为 `MEMORY_ENTRY_BLOCKED`，旧 DDR3 清单不能升级为产品 entry。
旧状态仍为 `DDR_ENTRY_READY=0 / BLOCKED`；
...
| DDR | S3 当前仍直接连接 `axi_ddr_behavioral` 容量占位模型；该模型明确没有 init/calibration/refresh/PHY/ECC，并将超出仿真 backing store 的地址折返。`docs/block_specs/ddr3_spec.md` v1.0 已冻结 AXI/APB/clock-reset/DFI/error contract；`SOC_APB_DDRCTRL_BASE=0x4000_6000` 及寄存器宏已进入 `soc_config.vh`；`docs/ddr_integration_inputs.md` 登记实现入口 | `xbar_ddr` 地址/边界/读写 unit 和 product MMU firmware 的 behavioral DDR store/load 均通过；`make ddr-contract-entry-audit DDR_ENTRY_AUDIT_DIR=build/unit_tb/ddr_contract_entry_v1` 的契约一致性检查通过，但结果明确为 `BLOCKED` | DDR 行仍只能是 `BLOCK_VERIFIED` 的 fabric/behavioral memory evidence。PHY/IP、DRAM part/timing file、real memory model 和 WDT budget 均缺失，controller/PHY RTL、DFI、校准/刷新状态、ECC、真实 backpressure/init failure 和 DDR boot 均未实现，属于 P0 blocker |
...
| 2026-08-02 | `integration/function-contract` | `make ddr-contract-entry-audit DDR_ENTRY_AUDIT_DIR=build/unit_tb/ddr_contract_entry_profile_c` | PASS：契约、ASIC Profile C 和输入计划检查通过；总体结果为 `BLOCKED`（预期） | 已记录用户选择 Profile C，并将 C1 DDR4/C2 LPDDR4/4X 设为下一项决策；旧 DDR3 清单降为 legacy prototype 边界，未授权产品 memory controller。报告：`build/unit_tb/ddr_contract_entry_profile_c/ddr_contract_entry_report.md` |
...
| 优先级 | 问题 | 对计划的影响 | 处理条件 |
|---|---|---|---|
| P0 | 产品 boot、DDR 和 QSPI 尚未闭合：Boot ROM 复位、普通与 refill/invalid BEV-EBase vector、IP-based vectored interrupt、最小 BEV MMU firmware、单一 EBase `Mod` recovery、development manifest header/CRC-to-SRAM handoff，以及 controller/AXI stall-to-DBE 和 QSPI timeout status 观测已通过；ASIC Profile C 已选但 `C-MEM-01..08` 仍未登记，旧 `DDR-IN-01..08` 仅为 legacy DDR3 边界，`ddr-contract-entry-audit` 已确认契约一致但 `DDR_ENTRY_READY=0`；PHY/IP、DRAM part/timing file、real memory model、WDT budget 和 RTL 仍未实现；生产 ROM/signature、原始 SPI 无响应检测、cache-error/EIC policy、QSPI/U-Boot/Linux 也未闭合 | SoC 已有受限的 reset-to-development-stage-1、behavioral DDR store/load、XIP transport-stall failure 和软件可读 timeout evidence，但无可发布的 secure boot 或产品主存，不能称商用 SoC | 先在 C1 DDR4 与 C2 LPDDR4/4X 中二选一，重建对应 memory contract，再完成 `C-MEM-01..08` 输入登记；之后实现真实 controller/PHY、APB status、init/calibration/refresh、bounded AXI error path 和 no-preload boot gate；并继续实现完整 runtime exception/cache-error policy、真正 QSPI command/FIFO/四线控制器、boot-status/WDT 与 production handoff，分别验证。 |
