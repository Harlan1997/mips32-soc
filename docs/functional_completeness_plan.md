...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。产品 XIP read 以默认 512-cycle AXI guard 限时，uncached 非 OKAY response 经 CPU 转 IBE/DBE，cached refill/writeback 错误经 CacheErr sideband 转 ExcCode=30；开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1。`qspi_apb_integration` 已接入外设 APB，保留旧 status map，并以 `+0x20` 暴露 command/FIFO；command CS 有效时由 SoC mux 接管共享单线 SPI pins；`spi_flash_behavioral` 可作为当前 command slice 的仿真 SPI 端点 | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests、AXI timeout guard、I/D cache error unit、CPU CacheErr/ERL directed gate、`product-cacheerr-gate` 以及有效、11 条 header/CRC failure 和 timeout-to-DBE manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过；`qspi-cmd-behavioral-gate` 覆盖 APB LUT、status RX、RX/TX FIFO、24-bit address、x4 data、CS/SCLK、busy error、IRQ W1C 和 soft reset；扩展 `qspi-status-integration` 经 AXI→APB 配置/触发 `0x06` 并在共享 pins 捕获 8 bit、done/W1C；新增 `qspi-flash-behavioral-gate` 经 `0x03` 读、`0x06` WREN、`0x02` NOR page-program 和再次读回；SoC smoke 与统一 RTL frontend `3/3` 通过 | 已达到有限 `SOC_INTEGRATED` APB/x1 command slice，且有 vendor-neutral flash protocol endpoint 证据，但不是完整 QSPI 产品路径。仍无 AXI/XIP command integration、商用 flash 许可模型/板级模型、quad pad/PHY、erase/program production path、production ROM/signature、ECC/完整软件错误分类、原始 SPI 无响应检测、DDR init 或 U-Boot boot，仍为 P0 blocker |
...
| 2026-08-02 | `integration/function-contract` QSPI vendor-neutral flash endpoint slice | `make qspi-flash-behavioral-gate QSPI_FLASH_BEHAVIORAL_DIR=/tmp/qspi_flash_behavioral3` | PASS：`REGRESSION_TEST_SUCCESS qspi_flash_behavioral` | `qspi_apb_integration` 通过共享 x1 pins 连接 `spi_flash_behavioral`；`0x03` 读回 `DE AD BE EF`，`0x06` 保持 WEL，`0x02` 对空白页执行 NOR `1->0` page-program，再次 `0x03` 读回 `CA FE BA BE`。模型仅为 vendor-neutral 仿真 endpoint，不能升级为商用 flash/PHY/AXI XIP 或 production erase/program 证据。 |
...
| P1 | 完成 QSPI 从有限 SoC command slice 到完整 SoC/boot 集成，并补齐 UART pad/driver path 和 boot failure 分类 | QSPI command/FIFO/24-bit address/x4 data 为 `BLOCK_VERIFIED (vendor-neutral)`；APB/x1 command 已达到有限 `SOC_INTEGRATED`，vendor-neutral SPI flash endpoint gate 已通过，但 AXI XIP/quad pad/真实 PHY/商用 flash/boot 未闭合；UART RTL pins、IRQ wiring、外部 RX waveform 和 SoC RX/PIC/RBR behavioral gate 已通过，pad-mux/板级/量产 RX driver 仍缺 | 当前 QSPI block + APB/SoC x1 + SPI flash behavioral + RTL frontend + SoC smoke PASS；后续需 AXI/XIP、商用 flash model/板级模型、quad pad wrapper、erase/program/boot SoC gate；UART 需 pad/driver/板级 gate |
