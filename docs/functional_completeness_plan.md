...
| Product TLB/MMU boot slice | `SOC_PRODUCT_BOOT_ENABLE=1` 与 `SOC_MMU_ENABLE=1` 下，CPU 保留 TLB lookup miss/invalid 的来源位；miss 选 `BFC0_0200`/`EBase`，invalid 保持 `BFC0_0380`/`EBase+0x180`；最小 Boot ROM linker、BEV refill handler、wired kseg2-APB 映射、动态 DDR refill，以及复制到 SRAM 的 EBase `Mod` handler 已新增 | 完整 SoC firmware directed 通过：I-side 覆盖两个 BEV 模式的 miss/invalid，D-side 覆盖 BEV=1 miss/invalid；两个 firmware gate 覆盖 `TLBWI`/`Wired`、DTLB refill/`ERET` retry、DDR/APB，以及 EBase `Mod` precise-state 检查、`D=1` 修复和 retry。不含完整 kseg0 runtime、vectored/cache-error 或 kernel boot，不能标为 MMU 产品完成 |
| Development manifest handoff | Boot ROM 经实际单线 SPI XIP 读取固定 64-byte `SOC1` development manifest，CRC32 校验后把 stage-1 拷贝至 Boot SRAM 并跳转 `0x8000_1000`；同时修复 SPI 读相位、MMU C=2 D-cache 属性路由和长路径镜像截断 | 完整 SoC directed gate 覆盖有效镜像，及 11 条单字段 header/CRC 失败镜像；不预加载 SRAM、不使用 `axi_flash_image_model`，并拒绝未加载的全 `FF` fixture，可由独立 Make target 和 block aggregate 重跑 | 仅为 `BLOCK_VERIFIED` 的 development boot 子集。没有签名、controller-error/timeout negative、QSPI 写擦/四线、DDR handoff、boot-status/WDT 或生产 ROM artifact |
...
|---|---|---|---|
| CPU/CP0 | 已接入；默认 `SOC_MMU_ENABLE=0`；产品模式区分 TLB miss refill 与 invalid/general 的 BEV/EBase 向量 | smoke 与 Phase 3A/3B CPU/CP0 gate 通过；产品 directed 覆盖 I-side BEV=1/0 miss/invalid、D-side BEV=1 miss/invalid，以及 EBase `Mod` precise state/recovery | refill/invalid 与最小 kernel-mode `Mod` recovery 子集已验证；vectored interrupt、cache-error、完整 Modified policy、ISA reference/compliance 和 MMU 产品启动仍未闭合 |
...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1 | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests，以及有效和 11 条 header/CRC failure manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过 | 真实单线 XIP 具备 development reset-to-handoff 与 manifest 字段拒绝证据；无 production ROM/signature、controller-error/timeout、QSPI command/FIFO/erase/program/四线、DDR init 或 U-Boot boot，仍为 P0 blocker |
...
- 已建立 `docs/boot_memory_contract.md` v0.9，冻结候选 reset/vector、物理/虚拟地址图、镜像格式、失败行为和六个行为 gate。
- 第二至第五个 RTL/firmware 垂直切片已完成：TLB lookup miss 与 matching-invalid 的 vector 分派覆盖 I-side 两个 BEV 模式和 D-side BEV=1；最小产品 Boot ROM linker/BEV refill handler 进一步覆盖 wired kseg2-APB 映射、DTLB refill、`TLBWR`、寄存器恢复、`ERET` retry、DDR store/load 和 APB write；独立 gate 还证明 Boot ROM 把通用 handler 复制到 SRAM `EBase+0x180`，处理 precise `Mod`、将 `D=0` 改为 `D=1` 并 `ERET` retry；development manifest gate 则经实际 SPI XIP 完成 CRC 校验、Boot SRAM 拷贝和 kseg0 stage-1 handoff。仍只关闭向量路由、最小 BEV 启动链、单一 `Mod` recovery 和 development handoff 子集；完整 kseg0 runtime、向量化/Cache-error 异常、生产 QSPI、真实 DDR、WDT 和生产 boot status 未实现。
...
- `SOC_MMU_ENABLE=1`：最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping，以及 EBase `Mod` handler relocation/retry gate 已通过；继续完成完整 kseg0 runtime linker、invalid/ASID/page-table policy、vectored/cache-error 和 kernel-mode firmware gate。refill/invalid 的 EBase/BEV 向量路由已有 directed 证据。
...
历史 full signoff 的功能阶段均通过，coverage 阈值单独失败，保留为后续质量工作。**Phase 0 的 C1/C2 分支整理、Phase 2 boot/memory 架构冻结、Boot ROM 复位/普通向量、TLB refill/invalid 的 BEV-EBase 路由、最小 Boot ROM kseg1 linker/BEV refill/wired mapping、复制到 SRAM 的 EBase `Mod` recovery，以及经实际 SPI XIP 的 development manifest/CRC-to-kseg0-SRAM handoff 和 header/CRC rejection matrix 已完成。当前下一项是 controller-error/timeout 失败行为，并行定义完整 kseg0 runtime linker、invalid/ASID policy、vectored/cache-error policy；仍不是 coverage closure。**
...
| 优先级 | 问题 | 对计划的影响 | 处理条件 |
|---|---|---|---|
| P0 | 产品 boot、DDR 和 QSPI 尚未闭合：Boot ROM 复位、普通与 refill/invalid BEV-EBase vector、最小 BEV MMU firmware、单一 EBase `Mod` recovery，以及 development manifest header/CRC-to-SRAM handoff 已通过；controller-error/timeout、生产 ROM/signature、剩余 vector policy、QSPI/U-Boot/Linux、DDR controller/PHY 仍未实现 | SoC 已有受限的 reset-to-development-stage-1 证据，但无可发布的 secure boot 或产品主存，不能称商用 SoC | 在本文件 Phase 2 继续实现 controller-error/timeout、完整 runtime exception policy、QSPI、DDR、boot-status/WDT 与 production handoff，并分别验证。 |
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping 和 SRAM EBase `Mod` recovery 已通过，但产品主 runtime 尚未迁移到 kseg0，invalid/ASID/page-table、vectored/cache-error 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动及 `Mod` recovery 证据，但不能作为可启动的产品 OS 功能 | 建立完整 kseg0 runtime linker、exception policy 和 kernel firmware gate，再跑 exception regression。 |
