...
- `SOC_MMU_ENABLE=1`：最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified policy gate、软件页表/context-switch 子集、4-ASID process-pressure、EBase `Mod` handler relocation/retry gate、stage-1 kseg0 指令交接、20-word/stack runtime-depth gate、`.rodata/.data/.bss/stack` runtime-layout slice，以及真实 cached-refill CacheErr handler/recovery gate 已通过；CACHE 四种 D-cache maintenance op 已有 CPU/块级证据；继续完成完整 runtime linker/loader ABI、TagLo/TagHi/SYNC、SoC page-table allocator/多进程压力、ECC/外部 EIC policy 和 kernel-mode firmware gate。refill/invalid 的 EBase/BEV 向量路由、CacheErr hardware vector 及 IP-based vectored interrupt 已有 directed 证据。
