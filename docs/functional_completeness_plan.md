...
| WDT/clock/reset | `apb_wdt` 已映射到 APB `0x4000_7000`；到期产生一次性 `wdt_reset`，`mips_soc_impl` 以 `soc_rst_n = rst_n & ~wdt_reset` 复位 SoC，WDT sticky status 留在 always-on 域 | WDT unit gate、外设子系统 AXI/APB gate、默认 SoC smoke 均通过；尚无 Boot ROM failure/status persistence gate | APB/reset path slice 已实现；boot-status register、失败码跨 watchdog reset 保留、启动失败 firmware 和板级 reset 观测仍未闭合 |
...
- 第二至第八个 RTL/firmware 垂直切片已完成：TLB lookup miss 与 matching-invalid 的 vector 分派覆盖 I-side 两个 BEV 模式和 D-side BEV=1；最小产品 Boot ROM linker/BEV refill handler 进一步覆盖 wired kseg2-APB 映射、DTLB refill、`TLBWR`、寄存器恢复、`ERET` retry、DDR store/load 和 APB write；独立 ASID gate 覆盖 4KB 非 Global 隔离、Global 跨 ASID、Invalid/Modified 分类；独立 gate 还证明 Boot ROM 把通用 handler 复制到 SRAM `EBase+0x180`，处理 precise `Mod`、将 `D=0` 改为 `D=1` 并 `ERET` retry；IP-based `Cause.IV/IntCtl.VS` vectored interrupt gate 已证明 IP1 到 `EBase+0x220`；development manifest gate 则经实际 SPI XIP 完成 CRC 校验、Boot SRAM 拷贝和 kseg0 stage-1 handoff，`product-kseg0-runtime-gate` 在 `SOC_MMU_ENABLE=1` 下确认入口 VA 到 SRAM PA 的取指转换；XIP guard 则将下游 AR/R stall 限时为 `SLVERR`，经 cache/CPU DBE 路径由 Boot ROM 记录 `DEAD_B007`；UART pins/IRQ slice 和 WDT APB/reset path slice 已有独立 gate。仍只关闭向量路由、最小 BEV 启动链、4KB ASID/异常分类、单一 `Mod` recovery、development handoff、kseg0 指令交接、AXI-side XIP stall 和 WDT reset pulse 子集；完整 kseg0 runtime 数据路径、cache-error/EIC 异常、生产 QSPI、真实 DDR、boot-status/失败码保留和 WDT boot-failure firmware 未实现。
...
- 外设：为已接入的 UART TX/RX/flow-control 完成 pad-mux、外部 RX waveform 和板级 gate；为已接入的 WDT 补 boot-status/失败码保留和 boot-failure firmware；补齐 GPIO/timer 产品软件驱动。
...
| P0 | UART RTL pins/IRQ wiring 已接入 `soc_top`，但 pad-mux/板级电气绑定、真实外部 RX waveform 和 RX firmware 仍未验收；WDT APB/reset pulse 已接入，但 boot-status/失败码保留和 boot-failure firmware 未验收 | 对外 serial I/O 仍缺板级证据；watchdog 只能证明 reset path，不能证明失败诊断和自动重启闭环 | 固化 UART pad contract 并补外部 RX gate；增加 boot-status register、跨 watchdog reset 的 stage/failure code retention，以及 WDT boot-failure gate。 |
