...
| P0 | 最小 Boot ROM kseg1 linker、BEV refill handler、wired mapping、4KB ASID/Global/Invalid/Modified gate、SRAM EBase `Mod` recovery、IP-based vectored interrupt 和 MMU-enabled kseg0 instruction/单次 data slice 已通过；page-table/ASID rollover、cache-error/EIC policy 和 kernel firmware 未验收；历史 prototype smoke timeout 的 fetch-path 根因已修复 | MMU/TLB 有最小启动、4KB ASID/异常分类、`Mod` recovery、CPU vector table 和有限 kseg0 handoff 证据，但不能作为可启动的产品 OS 功能 | 建立完整 kseg0 runtime linker/data layout、page-table/ASID rollover、exception policy 和 kernel firmware gate，再跑 exception regression。 |
