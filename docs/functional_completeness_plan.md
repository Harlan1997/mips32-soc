...
| `integration/function-contract` | 唯一功能集成线；本轮起始 HEAD `b77898e`，以 C2 `fcfc9c1` 为父线，已合入 C1 4-way I-cache、boot/memory 产品契约、Boot ROM/CP0 向量切片、kseg0 runtime ABI gate、UART 外部 RX waveform gate 和 SoC RX integration gate；QSPI behavioral 变更在本轮形成独立 commit | 当前验证和后续产品功能变更只在此线收敛，暂不直接推入 `master` |
...
| Flash/boot | `axi_spi_flash` 支持简单 SPI read XIP；独立只读 Boot ROM 已作为 S4 接入产品配置。产品 XIP read 以默认 512-cycle AXI guard 限时，uncached 非 OKAY response 经 CPU 转 IBE/DBE，cached refill/writeback 错误经 CacheErr sideband 转 ExcCode=30；开发 Boot ROM 可经产品 SPI 引脚读取 manifest/payload、CRC 校验、拷贝 Boot SRAM 并转交 kseg0 stage 1。`qspi_apb_integration` 已接入外设 APB，保留旧 status map，并以 `+0x20` 暴露 command/FIFO；command CS 有效时由 SoC mux 接管共享单线 SPI pins；`spi_flash_behavioral` 可作为当前 command slice 的仿真 SPI 端点；`qspi_pad_wrapper` 独立提供四线三态 pad boundary | Boot ROM burst/read-error/write-reject、无 SRAM preload 的首笔复位取指、response-PC 对齐的 reset branch、普通异常、TLB refill/invalid product directed tests、AXI timeout guard、I/D cache error unit、CPU CacheErr/ERL directed gate、`product-cacheerr-gate` 以及有效、11 条 header/CRC failure 和 timeout-to-DBE manifest handoff 均通过；生产 `axi_spi_flash` 的 pin-level `0x03`/24-bit address、连续读和写 `SLVERR` unit gate 通过；`qspi-cmd-behavioral-gate` 覆盖 APB LUT、status RX、RX/TX FIFO、24-bit address、x4 data、CS/SCLK、busy error、IRQ W1C 和 soft reset；扩展 `qspi-status-integration` 经 AXI→APB 配置/触发 `0x06` 并在共享 pins 捕获 8 bit、done/W1C；`qspi-flash-behavioral-gate` 经 `0x03` 读、`0x05` WEL、`0x06` WREN、`0x02` NOR page-program、`0x20` sector erase 和再次读回；`qspi-pad-wrapper-gate` 验证 x4 read/write pad direction/nibble 映射和高阻；SoC smoke 与统一 RTL frontend `3/3` 通过 | 已达到有限 `SOC_INTEGRATED` APB/x1 command slice，且有 vendor-neutral flash protocol endpoint 与 standalone tri-state pad boundary 证据，但不是完整 QSPI 产品路径。仍无 AXI/XIP command integration、商用 flash 许可模型/板级模型、SoC 四线 pad mux/真实 PHY、erase/program production path、production ROM/signature、ECC/完整软件错误分类、原始 SPI 无响应检测、DDR init 或 U-Boot boot，仍为 P0 blocker |
...
| P1 | 完成 QSPI 从有限 SoC command slice 到完整 SoC/boot 集成，并补齐 UART pad/driver path 和 boot failure 分类 | QSPI command/FIFO/24-bit address/x4 data 为 `BLOCK_VERIFIED (vendor-neutral)`；APB/x1 command 已达到有限 `SOC_INTEGRATED`，vendor-neutral SPI flash endpoint 和 standalone quad pad boundary gate 已通过，但 AXI XIP/SoC 四线 mux/真实 PHY/商用 flash/boot 未闭合；UART RTL pins、IRQ wiring、外部 RX waveform 和 SoC RX/PIC/RBR behavioral gate 已通过，pad-mux/板级/量产 RX driver 仍缺 | 当前 QSPI block + APB/SoC x1 + SPI flash behavioral + quad pad wrapper + RTL frontend + SoC smoke PASS；后续需 AXI/XIP、商用 flash model/板级模型、SoC quad pad mux/PHY、production erase/program/boot SoC gate；UART 需 pad/driver/板级 gate |
