...
| CPU/MMU hardware walker SoC AXI integration | `SOC_INTEGRATED` | `make mmu-hardware-walker-soc-gate` | `build/soc_test/mmu_hardware_walker_soc/sim.log` | Real SoC core-0 D-side AXI bridge covers two-level root/L2 reads, both halves of one VPN2, hardware refill/retry and zero software demand faults; default walker remains opt-in and production page-table ownership/OS policy remain outside this slice |
