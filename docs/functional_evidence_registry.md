...
| Dual-core cache notification path | `SOC_INTEGRATED` | `make dcache-coherency-gate dual-core-soc-gate` | `build/unit_tb/dcache_coherency/`, `build/soc_test/dual_core/` | The frozen opt-in IPI/TLB notification, timeout, stale-ack, busy-reentry, reset-isolation and exception-isolation slices pass; shared-memory firmware stress, snoop/refill collision and full memory-ordering proof remain outside this gate |
