...
### 2026-09-05 CP0 timer handler correlation

- Reused the existing RTL Linux simulator binary with all large artifacts under
  `/data/disk/tmp/mips32-soc/rtl-linux-timer-handler-20260905` and a 50M-cycle
  limit. The run exited normally (`SIM_RC=0`) with a bounded CP0 trace.
- The first long idle interval is architectural timer behavior: at cycle
  `47,982,855`, `Count=0x016e1483` crosses `Compare=0x016e1482`,
  `Cause.TI/IP7` is visible, and WAIT accepts the interrupt. The existing
  WAIT checker already verifies the matching EPC/BD/ERET contract.
- After the handler, Linux executes `mips_next_event()` in
  `third_party/linux/arch/mips/kernel/cevt-r4k.c` and writes
  `Compare=0x3297876f` at cycle `48,474,706`, then returns to `r4k_wait`.
  The large value is supplied by the kernel one-shot clockevent calculation;
  it is not generated by the RTL CP0 timer latch. No CP0/WAIT RTL change is
  justified by this capture. The remaining boundary is Linux scheduler/init
  handoff, followed by RTL userspace and full RTL/QEMU system differential.

### 2026-09-06 QEMU Linux differential CP0 identity alignment

- Added the opt-in `rtl-cp0-identity=on` machine property and enabled it only
  in `run_qemu_linux_differential_gate.sh`. Generic Linux and UHI guests keep
  QEMU's native Config1/2/3 behavior; bare-metal guests retain the existing
  RTL identity path.
- Rebuilt the custom QEMU machine under `/data/disk/tmp/mips32-soc` and ran a
  bounded 230K-retire replay. The Linux `decode_configs()` path now observes
  RTL-compatible Config1 `0xfe231180`, removing the previous CP0 geometry
  split.
- The replay still reports `retire=220818`, with RTL `mem_addr=0x00000001`
  and QEMU `mem_addr=0x88d9650c` in a later `lwl/sc` path. This is retained as
