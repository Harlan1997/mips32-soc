...
LINUX_VECTOR_LINE ?= 470410
LINUX_DELAY_TRACE ?= 0
LINUX_DELAY_TRACE_LIMIT ?= 256
LINUX_DELAY_TRACE_START ?= 892434e0
LINUX_DELAY_TRACE_END ?= 892434e4
...
LINUX_FAULT_TRACE_START ?= c0000000
LINUX_FAULT_TRACE_END ?= c0010000
...
LINUX_PC_TRACE_START ?=
LINUX_PC_TRACE_END ?=
LINUX_PC_TRACE_CYCLE_START ?= 0
LINUX_PC_TRACE_CYCLE_END ?= 0
LINUX_CP0_TRACE_LIMIT ?= 0
LINUX_RETIRE_TRACE ?= 0
LINUX_RETIRE_TRACE_MAX_RECORDS ?= 1000000
FW_NAME ?= soc_smoke
...
verification-foundation-gate: irq-contract-audit
...
.PHONY: irq-contract-audit

irq-contract-audit:
	@mkdir -p $(BUILD_DIR)/verification_foundation
	python3 scripts/check_irq_contract.py | tee $(BUILD_DIR)/verification_foundation/irq_contract_audit.log
...
mips-control-special2-gate:
	RUN_DIR=$(BUILD_DIR)/unit_tb/mips_control_special2 tb/unit/cpu_test/run_mips_control_special2.sh
...
linux-soc-contract-audit:
	python3 scripts/check_linux_soc_contract.py
...
rtl-linux-progress-gate:
	...
	LINUX_DELAY_TRACE=$(LINUX_DELAY_TRACE) \
	LINUX_DELAY_TRACE_LIMIT=$(LINUX_DELAY_TRACE_LIMIT) \
	LINUX_DELAY_TRACE_START=$(LINUX_DELAY_TRACE_START) \
	LINUX_DELAY_TRACE_END=$(LINUX_DELAY_TRACE_END) \
	LINUX_DELAY_TRACE_CYCLE_START=$(LINUX_DELAY_TRACE_CYCLE_START) \
	...
	LINUX_GPR_TRACE_CYCLE_START=$(LINUX_GPR_TRACE_CYCLE_START) \
	...
	LINUX_PANIC_TRACE=$(LINUX_PANIC_TRACE) \
	LINUX_PANIC_TRACE_LIMIT=$(LINUX_PANIC_TRACE_LIMIT) \
	LINUX_REFILL_TRACE=$(LINUX_REFILL_TRACE) \
	LINUX_PROGRESS_TRACE=$(LINUX_PROGRESS_TRACE) \
	LINUX_STALL_TRACE=$(LINUX_STALL_TRACE) LINUX_STALL_TRACE_LIMIT=$(LINUX_STALL_TRACE_LIMIT) LINUX_STALL_TRACE_CYCLE_START=$(LINUX_STALL_TRACE_CYCLE_START) LINUX_STALL_TRACE_PC=$(LINUX_STALL_TRACE_PC) \
	LINUX_REQUIRE_PROGRESS=$(LINUX_REQUIRE_PROGRESS) \
	LINUX_REQUIRE_USERSPACE=$(LINUX_REQUIRE_USERSPACE) \
	...
	LINUX_EBASE_TRACE_LIMIT=$(LINUX_EBASE_TRACE_LIMIT) LINUX_WB_TRACE_LIMIT=$(LINUX_WB_TRACE_LIMIT) \
	LINUX_MODE_TRACE=$(LINUX_MODE_TRACE) LINUX_MODE_TRACE_LIMIT=$(LINUX_MODE_TRACE_LIMIT) \
	LINUX_MODE_TRACE_CYCLE_START=$(LINUX_MODE_TRACE_CYCLE_START) \
	LINUX_MODE_TRACE_CYCLE_END=$(LINUX_MODE_TRACE_CYCLE_END) \
	LINUX_PC_TRACE=$(LINUX_PC_TRACE) LINUX_PC_TRACE_LIMIT=$(LINUX_PC_TRACE_LIMIT) \
	LINUX_PC_TRACE_RETIRE_ONLY=$(LINUX_PC_TRACE_RETIRE_ONLY) \
	LINUX_PC_TRACE_SYMBOL=$(LINUX_PC_TRACE_SYMBOL) \
	LINUX_PC_TRACE_START=$(LINUX_PC_TRACE_START) LINUX_PC_TRACE_END=$(LINUX_PC_TRACE_END) \
	LINUX_PC_TRACE_CYCLE_START=$(LINUX_PC_TRACE_CYCLE_START) \
	LINUX_PC_TRACE_CYCLE_END=$(LINUX_PC_TRACE_CYCLE_END) \
	LINUX_WAIT_TRACE=$(LINUX_WAIT_TRACE) LINUX_WAIT_TRACE_LIMIT=$(LINUX_WAIT_TRACE_LIMIT) \
	LINUX_TLB_TRACE=$(LINUX_TLB_TRACE) LINUX_TLB_TRACE_LIMIT=$(LINUX_TLB_TRACE_LIMIT) \
	LINUX_TLB_TRACE_CYCLE_START=$(LINUX_TLB_TRACE_CYCLE_START) LINUX_TLB_TRACE_CYCLE_END=$(LINUX_TLB_TRACE_CYCLE_END) \
	LINUX_VECTOR_TRACE=$(LINUX_VECTOR_TRACE) LINUX_VECTOR_TRACE_LIMIT=$(LINUX_VECTOR_TRACE_LIMIT) \
	LINUX_CACHEOP_TRACE_LIMIT=$(LINUX_CACHEOP_TRACE_LIMIT) \
	LINUX_CACHEOP_TRACE_LINE=$(LINUX_CACHEOP_TRACE_LINE) \
	LINUX_CP0_TRACE_LIMIT=$(LINUX_CP0_TRACE_LIMIT) \
	LINUX_VECTOR_LINE_TRACE=$(LINUX_VECTOR_LINE_TRACE) \
	LINUX_VECTOR_LINE_TRACE_LIMIT=$(LINUX_VECTOR_LINE_TRACE_LIMIT) \
	LINUX_VECTOR_LINE=$(LINUX_VECTOR_LINE) \
	LINUX_CACHE_OWNER_TRACE=$(LINUX_CACHE_OWNER_TRACE) \
	LINUX_CACHE_OWNER_TRACE_LIMIT=$(LINUX_CACHE_OWNER_TRACE_LIMIT) \
	LINUX_FAULT_TRACE=$(LINUX_FAULT_TRACE) \
	LINUX_FAULT_TRACE_LIMIT=$(LINUX_FAULT_TRACE_LIMIT) \
	LINUX_FAULT_TRACE_START=$(LINUX_FAULT_TRACE_START) \
	LINUX_FAULT_TRACE_END=$(LINUX_FAULT_TRACE_END) \
	LINUX_FAULT_TRACE_CYCLE_START=$(LINUX_FAULT_TRACE_CYCLE_START) \
...
qemu-system-linux-differential-gate: qemu-system-mips32-soc-ref
...
qemu-system-dma-reset-inflight-gate: qemu-system-mips32-soc-ref
...
qemu-system-fpu-rounding-differential-gate: qemu-system-mips32-soc-ref
	chmod +x tb/isa_ref/run_qemu_system_differential_gate.sh
	FW_TEST=fpu_rounding FW_DIR=$(BUILD_DIR)/firmware/fpu_rounding QEMU_CPU=24Kf RTL_TIMEOUT=120 RUN_DIR=$(BUILD_DIR)/isa_ref/qemu_system_fpu_rounding_differential RTL_VCS_EXTRA_ARGS='+define+SOC_FPU_ENABLE=1 +define+TB_SKIP_JTAG_RESET_STRESS' tb/isa_ref/run_qemu_system_differential_gate.sh
...
current-contract-signoff: soc-filelist-audit rtl-frontend-compile firmware firmwares micro-tlb-gate product-mmu-micro-tlb-gate interrupt-priority-gate cpu-irq-delay-slot-gate cpu-irq-mem-pending-gate mem-stage-gate dcache-parity-gate verification-foundation-gate irq-contract-audit cache-concurrency-gate l2-nonblocking-downstream-gate l1-nonblocking-gate l1-nonblocking-sync-gate l1-nonblocking-maintenance-compat-gate l1-nonblocking-cpu-error-gate l1-nonblocking-cpu-two-error-reset-gate mmu-page-frame-allocator-gate sva-gate qspi-vendor-neutral-complete-gate qspi-vendor-neutral-boot-gate ddr-contract-entry-audit ecc-secded-gate ddr4-complete-gate linux-gpio-userspace-gate
...
.PHONY: mmu-hardware-walker-ad-write-error-soc-gate
mmu-hardware-walker-ad-write-error-soc-gate:
	HW_WALKER=1 AD_WRITE_ERROR=1 FW_DIR=$(BUILD_DIR)/firmware/mmu_refill_hw_ad_write_error \
...
l1-nonblocking-cpu-complete-gate: l1-nonblocking-cpu-compat-gate l1-nonblocking-cpu-multi-gate l1-nonblocking-cpu-stress-gate l1-nonblocking-cpu-error-gate l1-nonblocking-cpu-two-error-gate l1-nonblocking-cpu-error-reset-gate l1-nonblocking-cpu-two-error-reset-gate l1-nonblocking-maintenance-compat-gate l1-nonblocking-maintenance-cpu-gate
	@mkdir -p $(BUILD_DIR)/soc_test/l1_nonblocking_cpu_complete
	@{ \
		echo '# L1 Nonblocking CPU Contract Completion Report'; \
		echo; \
		echo '- Result: PASS'; \
...
l1-nonblocking-maintenance-cpu-gate:
	chmod +x tb/soc_test/run_l1_nonblocking_maintenance_gate.sh
	RUN_DIR=$(BUILD_DIR)/soc_test/l1_nonblocking_maintenance \
	FW_DIR=$(BUILD_DIR)/firmware/l1_nonblocking_maintenance \
	tb/soc_test/run_l1_nonblocking_maintenance_gate.sh
...
qemu-system-l1-l2-nonblocking-differential-gate: qemu-system-mips32-soc-ref
	chmod +x tb/isa_ref/run_qemu_system_differential_gate.sh
	FW_TEST=qemu_system_l1_ddr FW_DIR=$(BUILD_DIR)/firmware/qemu_system_l1_ddr \
