...
module mips_tlb #(
    ...
    always @(*) begin
        ...
        for (j = TLB_ENTRIES - 1; j >= 0; j = j - 1) begin
            if (hit_vec[j]) begin
                // Any two valid matching entries are architecturally
                // ambiguous, including byte-for-byte duplicate entries.
                // Report MCheck rather than hiding the overlap behind the
                // priority encoder.
                if (probe_hit_r)
