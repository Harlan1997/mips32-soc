...
module mips_page_table_tlb_refill #(
    ...
    mips_page_table_walker #(.PAGE_MASK(PAGE_MASK)) u_walker (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin refill_pending <= 1'b0; resp_seen <= 1'b0; grant_seen <= 1'b0; va_q <= 0; asid_q <= 0; end
            ...
            // A successful walker response must always become a TLB write
            // transaction. Do not gate creation of the pending state on
            // tlb_wr_ready: a ready consumer would otherwise swallow the
            // response in the same cycle that the walker completes.
            if (resp_valid && !fault_valid && !refill_pending && !resp_seen) begin
                refill_pending <= 1'b1;
                resp_seen <= 1'b1;
            end
